...
title: Z.AI
x-i18n:
    generated_at: "2026-04-12T11:08:34Z"
    model: gpt-5.4
    provider: openai
...
- API: Z.AI Chat Completions (Bearer-автентифікація)
...
<Tabs>
  ...
  <Tab title="Явно вказаний регіональний endpoint">
    **Найкраще для:** користувачів, які хочуть примусово використовувати певний Coding Plan або загальний API-інтерфейс.
...
<AccordionGroup>
  <Accordion title="Попереднє розв’язання невідомих моделей GLM-5">
    Невідомі ідентифікатори `glm-5*` усе ще попередньо розв’язуються через шлях вбудованого провайдера
    шляхом синтезу метаданих, що належать провайдеру, з шаблону `glm-4.7`, коли ідентифікатор
    відповідає поточній формі сімейства GLM-5.
  </Accordion>

  <Accordion title="Потокова передача викликів інструментів">
    `tool_stream` увімкнено за замовчуванням для потокової передачі викликів інструментів Z.AI. Щоб вимкнути її:
  ...
  <Accordion title="Розуміння зображень">
    Вбудований Plugin Z.AI реєструє функцію розуміння зображень.

    | Властивість | Значення    |
    | ----------- | ----------- |
    | Модель      | `glm-4.6v`  |

    Розуміння зображень автоматично розв’язується з налаштованої автентифікації Z.AI —
  ...
  <Accordion title="Деталі автентифікації">
    ...
    - Використовуйте явні регіональні варіанти (`zai-coding-global`, `zai-coding-cn`, `zai-global`, `zai-cn`), якщо хочете примусово використовувати певний API-інтерфейс.
  </Accordion>
</AccordionGroup>
