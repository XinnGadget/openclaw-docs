---
read_when:
    - code_execution을 활성화하거나 구성하려는 경우
    - 로컬 셸 액세스 없이 원격 분석을 원하는 경우
...
이는 로컬 [`exec`](/tools/exec)와는 다릅니다.

- `exec`는 자신의 머신이나 노드에서 셸 명령을 실행합니다
- `code_execution`은 xAI의 원격 샌드박스에서 Python을 실행합니다

`code_execution`은 다음 용도로 사용하세요.
...
로컬 파일, 자신의 셸, 자신의 리포지토리 또는 페어링된
기기가 필요할 때는 사용하지 마세요. 그럴 때는 [`exec`](/tools/exec)를 사용하세요.
...
이 도구는 내부적으로 단일 `task` 매개변수를 사용하므로, 에이전트는
전체 분석 요청과 모든 인라인 데이터를 하나의 프롬프트로 보내야 합니다.
...
- 최신 X 데이터를 사용하려면 먼저 [`x_search`](/tools/web#x_search)를 사용하세요.
