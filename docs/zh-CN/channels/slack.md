...
title: Slack
x-i18n:
    generated_at: "2026-04-05T20:43:22Z"
    model: gpt-5.4
    provider: openai
...
状态：已可用于生产环境，支持通过 Slack 应用集成进行私信和渠道通信。默认模式为 Socket Mode；也支持 HTTP Events API 模式。
  ...
  <Tab title="HTTP Events API 模式">
    <Steps>
      <Step title="为 HTTP 配置 Slack 应用">
        ...
        - 将 Event Subscriptions、Interactivity 和 Slash command 的 Request URL 都设为同一个 webhook 路径（默认 `/slack/events`）
...
<AccordionGroup>
  <Accordion title="Slack 应用清单示例" defaultOpen>
...
- 可选：如果你希望发送消息使用当前智能体身份（自定义 `username` 和图标），可添加 `chat:write.customize`。`icon_emoji` 使用 `:emoji_name:` 语法。

状态快照行为：

- Slack 账户检查会跟踪每项凭证对应的 `*Source` 和 `*Status`
  字段（`botToken`、`appToken`、`signingSecret`、`userToken`）。
- 状态可能为 `available`、`configured_unavailable` 或 `missing`。
...
<Tip>
对于操作/目录读取，如果已配置用户令牌，则可以优先使用用户令牌。对于写入，仍优先使用机器人令牌；只有当 `userTokenReadOnly: false` 且机器人令牌不可用时，才允许使用用户令牌执行写入。
</Tip>
...
<Tabs>
  ...
  <Tab title="渠道策略">
    ...
    运行时说明：如果 `channels.slack` 完全缺失（仅环境变量设置），运行时将回退到 `groupPolicy="allowlist"` 并记录警告（即使设置了 `channels.defaults.groupPolicy` 也是如此）。

    名称/ID 解析：

    - 当令牌访问允许时，渠道允许列表条目和私信允许列表条目会在启动时解析
    - 无法解析的渠道名称条目会按配置保留，但默认不会用于路由
    - 入站授权和渠道路由默认优先使用 ID；若要直接按用户名/slug 匹配，需要设置 `channels.slack.dangerouslyAllowNameMatching: true`
...
- `channels.slack.thread.initialHistoryLimit` 控制新线程会话启动时会抓取多少现有线程消息（默认 `20`；设为 `0` 可禁用）。
...
注意：`replyToMode="off"` 会禁用 Slack 中的**所有**回复线程行为，包括显式 `[[reply_to_*]]` 标签。这与 Telegram 不同，在 Telegram 中，即使在 `"off"` 模式下也仍会遵循显式标签。之所以不同，是因为平台线程模型不同：Slack 线程会将消息隐藏在渠道主流之外，而 Telegram 回复仍会显示在主聊天流中。
...
- 布尔型 `channels.slack.streaming` 会自动迁移到 `channels.slack.nativeStreaming`。

## 输入中表情回应回退

`typingReaction` 会在 OpenClaw 处理回复期间，为入站 Slack 消息添加一个临时表情回应，并在运行结束后移除。在非线程回复场景中，这最有用，因为线程回复会使用默认的“正在输入...”状态指示器。
...
- 该表情回应为尽力而为，回复完成或失败路径结束后会自动尝试清理。

## 媒体、分块与投递

<AccordionGroup>
  <Accordion title="入站附件">
    Slack 文件附件会从 Slack 托管的私有 URL 下载（基于令牌认证的请求流程），并在抓取成功且尺寸限制允许时写入媒体存储。
  ...
  <Accordion title="出站文本与文件">
    ...
    - 如果配置了 `channels.slack.mediaMaxMb`，则出站媒体上限遵循该值；否则渠道发送会使用媒体流水线中的 MIME 类型默认值
  </Accordion>
...
- 启用原生命令后，需要在 Slack 中注册对应的斜杠命令（`/<command>` 名称），但有一个例外：
  - 状态命令请注册 `/agentstatus`（Slack 保留了 `/status`）
- 如果未启用原生命令，你可以通过 `channels.slack.slashCommand` 运行单个已配置的斜杠命令。
- 原生参数菜单现在会自适应其渲染策略：
  - 最多 5 个选项：按钮块
  - 6-100 个选项：静态选择菜单
  - 超过 100 个选项：当可用交互选项处理器时，使用带异步选项过滤的外部选择
...
并且仍会针对目标对话会话（`CommandTargetSessionKey`）路由命令执行。
...
- 如果生成的交互块会超出 Slack Block Kit 限制，OpenClaw 会回退为原始文本回复，而不是发送无效的块负载。
...
- 当请求已经落在 Slack 中且审批 ID 类型为 `plugin:` 时，插件审批仍可通过同一个 Slack 原生按钮界面进行处理。
- 审批人授权仍会被强制执行：只有被识别为审批人的用户才能通过 Slack 批准或拒绝请求。

这使用与其他渠道相同的共享审批按钮界面。当你在 Slack 应用设置中启用了 `interactivity` 后，审批提示会直接在对话中渲染为 Block Kit 按钮。
当这些按钮存在时，它们就是主要的审批 UX；只有当工具结果表明聊天审批不可用或手动审批是唯一途径时，OpenClaw
才应包含手动 `/approve` 命令。

配置路径：

- `channels.slack.execApprovals.enabled`
- `channels.slack.execApprovals.approvers`（可选；在可能时回退为 `commands.ownerAllowFrom`）
...
共享 `approvals.exec` 转发是独立功能。只有当 exec 审批提示还必须
路由到其他聊天或显式的带外目标时，才使用它。共享 `approvals.plugin` 转发也
是独立功能；当这些请求已经落在 Slack
中时，Slack 原生按钮仍可处理插件审批。

同一聊天中的 `/approve` 也可在已支持命令的 Slack 渠道和私信中使用。完整的审批转发模型，请参见 [Exec approvals](/zh-CN/tools/exec-approvals)。

## 事件与运行行为

- 消息编辑/删除/线程广播会映射为系统事件。
- 添加/移除表情回应事件会映射为系统事件。
- 成员加入/离开、渠道创建/重命名，以及添加/移除置顶事件都会映射为系统事件。
- 当启用 `configWrites` 时，`channel_id_changed` 可以迁移渠道配置键。
- 渠道 topic/purpose 元数据被视为不可信上下文，可注入到路由上下文中。
...
```
  ...
  <Accordion title="Socket 模式未连接">
    验证机器人令牌、应用令牌以及 Slack 应用设置中的 Socket Mode 是否已启用。

    如果 `openclaw channels status --probe --json` 显示 `botTokenStatus` 或
    `appTokenStatus: "configured_unavailable"`，说明 Slack 账户已完成
