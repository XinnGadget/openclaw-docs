...
- Configuração de modelo e autenticação (OAuth da assinatura OpenAI Code, Anthropic Claude CLI ou chave de API, além de opções de MiniMax, GLM, Ollama, Moonshot, StepFun e AI Gateway)
...
<Steps>
  <Step title="Detecção de configuração existente">
    ...
    - A CLI `--reset` usa por padrão `config+creds+sessions`; use `--reset-scope full` para também remover o workspace.
  ...
  <Step title="Gateway">
    ...
    - Caminho SecretRef para token em modo não interativo: `--gateway-token-ref-env <ENV_VAR>`.
  ...
  <Step title="Instalação do daemon">
    - macOS: LaunchAgent
      - Exige uma sessão de usuário autenticada; para modo headless, use um LaunchDaemon personalizado (não incluído).
    ...
    - Windows nativo: primeiro Tarefa Agendada
      - Se a criação da tarefa for negada, o OpenClaw usa como fallback um item de login por usuário na pasta Inicializar e inicia o gateway imediatamente.
      - Tarefas Agendadas continuam sendo a opção preferida porque fornecem melhor status do supervisor.
  ...
  <Step title="Verificação de integridade">
    ...
    - `openclaw status --deep` adiciona a sonda de integridade do gateway ativo à saída de status, incluindo sondas de canal quando suportadas.
  </Step>
  <Step title="Skills">
...
<Note>
Se nenhuma interface gráfica for detectada, o assistente imprime instruções de encaminhamento de porta por SSH para a Control UI em vez de abrir um navegador.
...
<AccordionGroup>
  ...
  <Accordion title="Assinatura OpenAI Code (reutilização do Codex CLI)">
    ...
    As credenciais reutilizadas do Codex CLI continuam sendo gerenciadas pelo Codex CLI; ao expirar, o OpenClaw
    relê essa origem primeiro e, quando o provedor consegue atualizá-la, grava
    a credencial atualizada de volta no armazenamento do Codex em vez de assumir o controle
    ele mesmo.
  </Accordion>
  ...
  <Accordion title="Chave de API xAI (Grok)">
    Solicita `XAI_API_KEY` e configura xAI como um provedor de modelo.
  </Accordion>
  ...
  <Accordion title="Ollama (Cloud e modelos abertos locais)">
    Solicita primeiro `Cloud + Local`, `Somente Cloud` ou `Somente Local`.
    `Somente Cloud` usa `OLLAMA_API_KEY` com `https://ollama.com`.
  ...
  <Accordion title="Provedor personalizado">
    ...
    - **Usar referência de segredo** (referência de ambiente ou referência de provedor configurado, com validação prévia)
    ...
    - `--custom-compatibility <openai|anthropic>` (opcional; o padrão é `openai`)
...
- Quando o onboarding começa a partir de uma opção de autenticação de provedor, o seletor de modelo prefere
  ...
  também corresponde às variantes de plano de codificação deles (`volcengine-plan/*`,
  `byteplus-plan/*`).
- Se esse filtro de provedor preferido ficar vazio, o seletor recorre ao
...
- `--secret-input-mode ref` ativa o modo de referência em vez do armazenamento de chave em texto simples.
...
- O modo de referência interativo executa uma validação prévia rápida antes de salvar.
  - Referências de ambiente: valida o nome da variável + valor não vazio no ambiente atual de onboarding.
...
- No modo não interativo, `--secret-input-mode ref` aceita apenas ambiente.
  - Defina a variável de ambiente do provedor no ambiente do processo de onboarding.
  - Sinalizadores de chave inline (por exemplo, `--openai-api-key`) exigem que essa variável de ambiente esteja definida; caso contrário, o onboarding falha imediatamente.
...
- As credenciais de autenticação do Gateway oferecem suporte a texto simples e SecretRef na configuração interativa:
...
<Note>
Dica para servidores e ambientes headless: conclua o OAuth em uma máquina com navegador e depois copie
...
- `session.dmScope` (o onboarding local define isso por padrão como `per-channel-peer` quando não está definido; valores explícitos existentes são preservados)
- `channels.telegram.botToken`, `channels.discord.token`, `channels.matrix.*`, `channels.signal.*`, `channels.imessage.*`
- Allowlists de canal (Slack, Discord, Matrix, Microsoft Teams) quando você opta por isso durante os prompts (os nomes são resolvidos para IDs quando possível)
- `skills.install.nodeManager`
  - O sinalizador `setup --node-manager` aceita `npm`, `pnpm` ou `bun`.
