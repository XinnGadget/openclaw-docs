...
title: 工具和插件
x-i18n:
    generated_at: "2026-04-06T01:01:08Z"
    model: gpt-5.4
    provider: openai
...
<Steps>
  <Step title="工具是智能体调用的内容">
    ...
    `web_search`、`message`）。OpenClaw 内置了一组**内置工具**，而且
    插件还可以注册其他工具。
  ...
  <Step title="Skills 教会智能体何时以及如何使用">
    ...
    Skills 为智能体提供上下文、约束，以及高效使用工具的分步指导。
...
| `apply_patch`                              | 多块文件补丁                                                  | [应用补丁](/zh-CN/tools/apply-patch)              |
...
对于图像工作，使用 `image` 进行分析，使用 `image_generate` 进行生成或编辑。如果你的目标是 `openai/*`、`google/*`、`fal/*` 或其他非默认图像提供商，请先配置该提供商的身份验证 / API 密钥。

对于音乐工作，使用 `music_generate`。如果你的目标是 `google/*`、`minimax/*` 或其他非默认音乐提供商，请先配置该提供商的身份验证 / API 密钥。

对于视频工作，使用 `video_generate`。如果你的目标是 `qwen/*` 或其他非默认视频提供商，请先配置该提供商的身份验证 / API 密钥。

对于工作流驱动的音频生成，当某个插件（例如
ComfyUI）注册了它时，请使用 `music_generate`。这与 `tts`
不同，后者是文本转语音。

`session_status` 是 sessions 分组中的轻量级状态 / 回读工具。
它可以回答当前会话中类似 `/status` 的问题，并且可以
选择性设置按会话生效的模型覆盖；`model=default` 会清除此
覆盖。与 `/status` 一样，它可以从最新的转录使用记录中回填稀疏的
token / cache 计数器，以及当前活动运行时模型标签。

`gateway` 是仅所有者可用的 Gateway 网关运行时工具，用于执行 Gateway 网关操作：

- `config.schema.lookup`：在编辑前查看单个路径范围内的配置子树
- `config.get`：获取当前配置快照 + 哈希值
- `config.patch`：执行局部配置更新并重启
- `config.apply`：仅用于完整配置替换
- `update.run`：显式执行自更新 + 重启

对于局部更改，优先使用 `config.schema.lookup`，然后使用 `config.patch`。仅当你有意替换整个配置时，才使用
`config.apply`。
该工具还会拒绝更改 `tools.exec.ask` 或 `tools.exec.security`；
旧版 `tools.bash.*` 别名会归一化到相同的受保护 exec 路径。
...
- [音乐生成](/zh-CN/tools/music-generation) — 共享的 `music_generate` 工具，以及插件提供的工作流变体
...
`sessions_history` 返回的是有界的、经过安全过滤的回忆视图。它会从助手文本中去除
...
token，以及格式错误的来自 MiniMax 的工具调用 XML，然后再执行
脱敏 / 截断，并在可能的情况下使用超大行占位符，而不是把它作为
原始转录转储返回。
