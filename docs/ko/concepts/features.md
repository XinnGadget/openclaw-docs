---
read_when:
    - OpenClaw가 지원하는 전체 목록을 확인하고 싶을 때
...
<Columns>
  <Card title="채널" icon="message-square">
    단일 Gateway로 Discord, iMessage, Signal, Slack, Telegram, WhatsApp, WebChat 등을 지원합니다.
  </Card>
...
- allowlist와 페어링을 통한 DM 안전성

**에이전트:**

- 도구 스트리밍을 지원하는 임베디드 에이전트 런타임
...
- 사용자 지정 및 self-hosted provider 지원(vLLM, SGLang, Ollama, 그리고 모든 OpenAI 호환 또는 Anthropic 호환 엔드포인트)

**미디어:**

- 이미지, 오디오, 비디오, 문서를 입력과 출력으로 지원
- 공용 이미지 생성 및 비디오 생성 기능 표면
- 음성 메모 전사
...
- 페어링, 채팅, 음성, Canvas, 카메라, 장치 명령을 지원하는 Android 노드
