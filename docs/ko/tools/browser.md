...
- `browser.profiles.<name>.userDataDir`를 설정하면 기존 세션 프로필이 Brave 또는 Edge 같은 기본이 아닌 Chromium 사용자 프로필에 연결할 수 있습니다.
...
모든 명령은 특정 프로필을 대상으로 지정하기 위해 `--browser-profile <name>`을 받을 수 있습니다.
...
- `snapshot`:
  - `--format ai`(Playwright가 설치되어 있을 때 기본값): 숫자 ref(`aria-ref="<n>"`)가 포함된 AI 스냅샷을 반환합니다.
  ...
  - `--interactive`는 선택하기 쉬운 평면형 대화형 요소 목록을 출력합니다(작업 구동에 가장 적합).
  - `--labels`는 ref 라벨이 오버레이된 뷰포트 전용 스크린샷을 추가합니다(`MEDIA:<path>` 출력).
- `click`/`type` 등은 `snapshot`의 `ref`(숫자 `12` 또는 role ref `e12`)가 필요합니다.
  작업에는 의도적으로 CSS selector를 지원하지 않습니다.
...
- **Role 스냅샷(`e12` 같은 role ref)**: `openclaw browser snapshot --interactive`(또는 `--compact`, `--depth`, `--selector`, `--frame`)
  ...
  - 내부적으로 ref는 `getByRole(...)`(중복에는 `nth()` 추가)를 통해 확인됩니다.
...
- URL 대기(Playwright에서 지원하는 glob 사용 가능):
...
2. `click <ref>` / `type <ref>`를 사용합니다(대화형 모드에서는 role ref 권장)
3. 여전히 실패하면: `openclaw browser highlight <ref>`로 Playwright가 무엇을 대상으로 삼는지 확인합니다
4. 페이지 동작이 이상하면:
   - `openclaw browser errors --clear`
   - `openclaw browser requests --filter api --clear`
5. 심층 디버깅이 필요하면 trace를 기록합니다:
   - `openclaw browser trace start`
   - 문제를 재현합니다
...
`--json`은 스크립팅 및 구조화된 도구 처리를 위한 옵션입니다.
...
JSON의 Role 스냅샷에는 `refs`와 작은 `stats` 블록(lines/chars/refs/interactive)이 포함되어, 도구가 payload 크기와 밀도를 판단할 수 있습니다.

## 상태 및 환경 설정값

이 설정들은 “사이트가 X처럼 동작하게 만들기” 워크플로에 유용합니다.
...
- Gateway/node host는 비공개 상태로 유지하세요(루프백 또는 tailnet 전용).
...
[Browser troubleshooting](/ko/tools/browser-linux-troubleshooting)을 참조하세요.

WSL2 Gateway + Windows Chrome 분리 호스트 설정은
[WSL2 + Windows + remote Chrome CDP troubleshooting](/ko/tools/browser-wsl2-windows-remote-cdp-troubleshooting)을 참조하세요.
...
- **탐색 SSRF 차단**은 브라우저 제어 플레인은 정상이나, 페이지 탐색 대상이 정책에 의해 거부된다는 뜻입니다.
...
- 탐색 SSRF 차단:
  - `open`, `navigate`, 스냅샷 또는 탭 열기 흐름이 브라우저/네트워크 정책 오류로 실패하지만 `start`와 `tabs`는 계속 작동함

다음 최소 절차를 사용해 두 경우를 구분하세요.
...
- `start`는 성공했지만 `tabs`가 실패하면 제어 플레인이 여전히 비정상입니다. 이를 페이지 탐색 문제가 아니라 CDP 연결 가능성 문제로 취급하세요.
- `start`와 `tabs`는 성공하지만 `open` 또는 `navigate`가 실패하면 브라우저 제어 플레인은 정상이며, 실패 원인은 탐색 정책 또는 대상 페이지에 있습니다.
...
- 로컬 루프백 `openclaw` 관리형 프로필의 경우, CDP 상태 확인은 OpenClaw 자체 로컬 제어 플레인에 대한 브라우저 SSRF 연결 가능성 강제를 의도적으로 건너뜁니다.
- 탐색 보호는 별개입니다. `start`나 `tabs`가 성공했다고 해서 이후의 `open` 또는 `navigate` 대상이 허용된다는 뜻은 아닙니다.

보안 지침:

- 기본적으로 브라우저 SSRF 정책을 완화하지 마세요.
- 광범위한 사설 네트워크 액세스보다 `hostnameAllowlist` 또는 `allowedHostnames` 같은 좁은 호스트 예외를 우선 사용하세요.
- `dangerouslyAllowPrivateNetwork: true`는 사설 네트워크 브라우저 액세스가 필요하고 검토된, 의도적으로 신뢰된 환경에서만 사용하세요.

예시: 탐색은 차단되지만 제어 플레인은 정상

- `start` 성공
- `tabs` 성공
- `open http://internal.example` 실패

이는 보통 브라우저 시작은 정상이며 탐색 대상에 대해 정책 검토가 필요함을 의미합니다.

예시: 탐색 이전 단계에서 시작이 차단됨

- `start`가 `not reachable after start`로 실패
- `tabs`도 실패하거나 실행할 수 없음

이는 페이지 URL 허용 목록 문제가 아니라 브라우저 실행 또는 CDP 연결 가능성 문제를 가리킵니다.
...
- `browser`는 다음을 받을 수 있습니다.
  - `profile`: 이름 있는 브라우저 프로필(openclaw, chrome 또는 원격 CDP)을 선택
  - `target` (`sandbox` | `host` | `node`): 브라우저가 위치한 대상을 선택
  ...
  - 브라우저 사용이 가능한 node가 연결되어 있으면 `target="host"` 또는 `target="node"`로 고정하지 않는 한 도구가 자동으로 해당 node로 라우팅될 수 있습니다.

이렇게 하면 에이전트 동작이 결정론적으로 유지되고 취약한 selector를 피할 수 있습니다.

## 관련 항목

- [Tools Overview](/ko/tools) — 사용 가능한 모든 에이전트 도구
- [Sandboxing](/ko/gateway/sandboxing) — 샌드박스 환경에서의 브라우저 제어
- [Security](/ko/gateway/security) — 브라우저 제어 위험 및 보안 강화
