...
状态：已可用于生产环境，支持通过 Slack 应用集成进行私信和渠道消息。默认模式为 Socket Mode；也支持 HTTP 请求 URL。
...
```
        ...
        为每个账户指定不同的 `webhookPath`（默认是 `/slack/events`），以避免注册发生冲突。
        </Note>
...
```
  ...
  <Accordion title="可选的用户令牌作用域（读取操作）">
    如果你配置了 `channels.slack.userToken`，常见的读取作用域包括：
...
- `userToken`（`xoxp-...`）仅可通过配置提供（没有环境变量回退），并且默认是只读行为（`userTokenReadOnly: true`）。
...
- `configured_unavailable` 表示该账户通过 SecretRef 或其他非内联密钥源完成了配置，但当前命令/运行时路径无法解析实际值。
- 在 HTTP 模式下，会包含 `signingSecretStatus`；在 Socket Mode 下，所需配对是 `botTokenStatus` + `appTokenStatus`。

<Tip>
对于 actions/目录读取，如果配置了用户令牌，则可以优先使用它。对于写入，仍优先使用 bot 令牌；只有在 `userTokenReadOnly: false` 且 bot 令牌不可用时，才允许使用用户令牌进行写入。
</Tip>
...
<Tabs>
  ...
  <Tab title="渠道策略">
    ...
    运行时说明：如果 `channels.slack` 完全缺失（仅使用环境变量进行设置），运行时会回退到 `groupPolicy="allowlist"` 并记录一条警告（即使设置了 `channels.defaults.groupPolicy` 也是如此）。

    名称/ID 解析：

    - 当令牌访问允许时，渠道 allowlist 条目和私信 allowlist 条目会在启动时解析
    - 无法解析的渠道名称条目会按配置原样保留，但默认不会用于路由
    - 入站授权和渠道路由默认优先基于 ID；直接按用户名/slug 匹配需要 `channels.slack.dangerouslyAllowNameMatching: true`
  ...
  <Tab title="提及和渠道用户">
    ...
    每渠道控制（`channels.slack.channels.<id>`；名称仅通过启动时解析或 `dangerouslyAllowNameMatching`）：
...
- `channels.slack.thread.initialHistoryLimit` 控制新线程会话启动时抓取多少现有线程消息（默认 `20`；设为 `0` 可禁用）。
- `channels.slack.thread.requireExplicitMention`（默认 `false`）：当设为 `true` 时，会抑制隐式线程提及，因此 bot 仅会在它已参与线程的情况下，响应线程内显式的 `@bot` 提及。否则，在 bot 已参与的线程中回复会绕过 `requireMention` 门控。
...
注意：`replyToMode="off"` 会禁用 Slack 中**所有**回复线程功能，包括显式的 `[[reply_to_*]]` 标签。这与 Telegram 不同，在 Telegram 中，即使在 `"off"` 模式下也仍会遵循显式标签。这种差异反映了平台线程模型的不同：Slack 线程会将消息隐藏在渠道之外，而 Telegram 回复仍会显示在主聊天流中。
...
`typingReaction` 会在 OpenClaw 处理回复时，向入站 Slack 消息添加一个临时反应，并在运行完成后将其移除。这在非线程回复场景中特别有用，因为线程回复会使用默认的“正在输入...”状态指示器。
...
- 该反应是尽力而为的，系统会在回复完成或失败路径结束后自动尝试清理。
...
<AccordionGroup>
  ...
  <Accordion title="出站文本和文件">
    ...
    - 如果配置了 `channels.slack.mediaMaxMb`，出站媒体上限会遵循它；否则渠道发送会使用媒体流水线中的 MIME 类型默认值
  </Accordion>
...
斜杠命令在 Slack 中可以显示为单个已配置命令，或多个原生命令。配置 `channels.slack.slashCommand` 以更改命令默认值：
...
原生命令需要对你的 Slack 应用进行[清单更改](#optional-native-slash-commands)，并通过 `channels.slack.commands.native: true` 启用；或者在全局配置中改用 `commands.native: true`。
...
原生参数菜单使用一种自适应渲染策略，在分发所选选项值之前会显示确认模态框：
...
- 这是 Slack 专用 UI。其他渠道不会将 Slack Block Kit 指令翻译为自己的按钮系统。
...
- Exec 审批使用 `channels.slack.execApprovals.*` 来进行原生私信/渠道路由。
- 当请求已经到达 Slack，且审批 ID 类型为 `plugin:` 时，插件审批也可以通过同样的 Slack 原生按钮界面完成。
...
- `channels.slack.execApprovals.approvers`（可选；如有可能会回退到 `commands.ownerAllowFrom`）
...
设置 `enabled: true` 则会在审批人可解析时强制启用原生审批。
...
路由到其他聊天或显式的带外目标时才使用它。共享的 `approvals.plugin` 转发也
是独立的；当这些请求已经到达 Slack 时，Slack 原生按钮仍然可以处理插件审批。

同聊天中的 `/approve` 在已经支持命令的 Slack 渠道和私信中也可用。有关完整的审批转发模型，请参见[Exec 审批](/zh-CN/tools/exec-approvals)。
...
- 在线程起始消息和初始线程历史上下文注入时，会在适用情况下按已配置的发送者 allowlist 进行过滤。
- Block actions 和模态交互会发出结构化的 `Slack interaction: ...` 系统事件，并带有丰富的负载字段：
  - block actions：选中值、标签、选择器值以及 `workflow_*` 元数据
...
```
  ...
  <Accordion title="Socket 模式未连接">
    验证 bot 和 app 令牌，以及 Slack 应用设置中是否启用了 Socket Mode。
  ...
  <Accordion title="原生/斜杠命令未触发">
    ...
    另外还要检查 `commands.useAccessGroups` 以及渠道/用户 allowlist。
