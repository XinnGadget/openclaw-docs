---
read_when:
    - 웹 검색에 Perplexity Search를 사용하려고 할 때
...
이 API는 `title`, `url`, `snippet` 필드가 있는 구조화된 결과를 반환합니다.
...
이미 OpenRouter를 Perplexity Sonar에 사용하고 있었다면, `provider: "perplexity"`를 유지하고 Gateway 환경에 `OPENROUTER_API_KEY`를 설정하거나 `plugins.entries.perplexity.config.webSearch.apiKey`에 `sk-or-...` 키를 저장하세요.
...
**config를 통해:** `openclaw configure --section web`을 실행하세요. 키는
...
gateway 설치의 경우
`~/.openclaw/.env`(또는 서비스 환경)에 넣으세요. 자세한 내용은 [Env vars](/help/faq#env-vars-and-env-loading)를 참조하세요.

`provider: "perplexity"`가 구성되어 있고 Perplexity 키 SecretRef가 해석되지 않으며 env 대체도 없으면, 시작/리로드는 빠르게 실패합니다.
...
| `domain_filter` | 도메인 allowlist/denylist 배열(최대 20개) |
...
- 여기서 `count`는 호환성 전용이며, 응답은 여전히 N개 결과 목록이 아니라 인용이 포함된 하나의 합성 답변입니다
...
- 필터당 최대 20개 도메인
- 동일한 요청에서 allowlist와 denylist를 혼합할 수 없음
- denylist 항목에는 `-` 접두사를 사용(예: `["-reddit.com"]`)

## 참고

- Perplexity Search API는 구조화된 웹 검색 결과(`title`, `url`, `snippet`)를 반환합니다
- OpenRouter 또는 명시적인 `plugins.entries.perplexity.config.webSearch.baseUrl` / `model`을 설정하면 호환성을 위해 Perplexity가 다시 Sonar chat completions로 전환됩니다
- Sonar/OpenRouter 호환성은 구조화된 결과 행이 아니라 인용이 포함된 하나의 합성 답변을 반환합니다
- 결과는 기본적으로 15분 동안 캐시됩니다(`cacheTtlMinutes`로 구성 가능)

전체 `web_search` 구성은 [Web tools](/tools/web)를 참조하세요.
자세한 내용은 [Perplexity Search API docs](https://docs.perplexity.ai/docs/search/quickstart)를 참조하세요.
