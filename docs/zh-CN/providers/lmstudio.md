...
LM Studio 是一款友好且功能强大的应用，可让你在自己的硬件上运行开放权重模型。它支持运行 llama.cpp（GGUF）或 MLX 模型（Apple Silicon）。提供 GUI 安装包，也提供无头守护进程（`llmster`）。有关产品和设置文档，请参阅 [lmstudio.ai](https://lmstudio.ai/)。
...
如果你使用的是应用，请确保已启用 JIT，以获得流畅体验。了解更多信息，请参阅 [LM Studio JIT and TTL guide](https://lmstudio.ai/docs/developer/core/ttl-and-auto-evict)。
...
`--custom-api-key` 仍然保留以兼容旧用法，但对于 LM Studio，优先使用 `--lmstudio-api-key`。
