...
summary: Configuración y comportamiento en tiempo de ejecución de Slack (Socket Mode + URL de solicitudes HTTP)
title: Slack
x-i18n:
    generated_at: "2026-04-07T05:02:52Z"
    model: gpt-5.4
    provider: openai
...
Estado: listo para producción para MD + canales mediante integraciones de aplicaciones de Slack. El modo predeterminado es Socket Mode; también se admiten URL de solicitudes HTTP.
...
```
        ...
        Respaldo de variables de entorno (solo cuenta predeterminada):
...
- Los tokens de configuración tienen prioridad sobre el respaldo de variables de entorno.
- El respaldo de variables de entorno `SLACK_BOT_TOKEN` / `SLACK_APP_TOKEN` se aplica solo a la cuenta predeterminada.
- `userToken` (`xoxp-...`) es solo de configuración (sin respaldo en variables de entorno) y usa de forma predeterminada comportamiento de solo lectura (`userTokenReadOnly: true`).
...
- `configured_unavailable` significa que la cuenta está configurada mediante SecretRef
  u otra fuente de secretos no integrada, pero la ruta actual de comando/ejecución
...
<Tabs>
  ...
  <Tab title="Política de canal">
    ...
    La lista de permitidos de canales vive en `channels.slack.channels` y debe usar IDs de canal estables.

    Nota de tiempo de ejecución: si falta `channels.slack` por completo (configuración solo con variables de entorno), el tiempo de ejecución vuelve a `groupPolicy="allowlist"` y registra una advertencia (incluso si `channels.defaults.groupPolicy` está definido).

    Resolución de nombre/ID:

    - las entradas de lista de permitidos de canales y de MD se resuelven al inicio cuando el acceso mediante token lo permite
    - las entradas no resueltas de nombre de canal se conservan tal como fueron configuradas, pero se ignoran para el enrutamiento de forma predeterminada
  ...
  <Tab title="Menciones y usuarios del canal">
    Los mensajes en canales están restringidos por mención de forma predeterminada.
    ...
    - formato de clave de `toolsBySender`: `id:`, `e164:`, `username:`, `name:` o comodín `"*"`
      (las claves heredadas sin prefijo siguen mapeándose solo a `id:`)
...
- Con el valor predeterminado `session.dmScope=main`, los MD de Slack se agrupan en la sesión principal del agente.
...
- `channels.slack.thread.requireExplicitMention` (predeterminado `false`): cuando es `true`, suprime las menciones implícitas en hilos para que el bot solo responda a menciones explícitas de `@bot` dentro de los hilos, incluso cuando el bot ya participó en el hilo. Sin esto, las respuestas en un hilo en el que participó el bot omiten la compuerta `requireMention`.
...
- Usa `""` para deshabilitar la reacción para la cuenta de Slack o de forma global.
...
- `progress`: muestra texto de estado del progreso mientras genera y luego envía el texto final.
...
- Debe haber disponible un hilo de respuesta para que aparezca el streaming nativo de texto. La selección del hilo sigue obedeciendo `replyToMode`. Sin uno, se usa la vista previa normal del borrador.
...
<AccordionGroup>
  <Accordion title="Archivos adjuntos entrantes">
    Los archivos adjuntos de Slack se descargan desde URL privadas alojadas por Slack (flujo de solicitud autenticado por token) y se escriben en el almacén multimedia cuando la recuperación tiene éxito y los límites de tamaño lo permiten.

    El límite de tamaño entrante en tiempo de ejecución usa `20MB` de forma predeterminada, a menos que se sobrescriba con `channels.slack.mediaMaxMb`.
  ...
  <Accordion title="Texto y archivos salientes">
    ...
    - el límite de medios salientes sigue `channels.slack.mediaMaxMb` cuando está configurado; de lo contrario, los envíos del canal usan los valores predeterminados por tipo MIME del pipeline multimedia
  </Accordion>
...
- Habilita los controladores de comandos nativos de Slack con `channels.slack.commands.native: true` (o el valor global `commands.native: true`).
...
- Los menús nativos de argumentos ahora adaptan su estrategia de renderizado:
  ...
  - más de 100 opciones: selección externa con filtrado asíncrono de opciones cuando están disponibles los controladores de opciones de interactividad
  - si los valores de opciones codificados superan los límites de Slack, el flujo vuelve a botones
- Para cargas largas de opciones, los menús de argumentos de comandos slash usan un diálogo de confirmación antes de enviar un valor seleccionado.
...
- Si los bloques interactivos generados superan los límites de Slack Block Kit, OpenClaw vuelve al texto de respuesta original en lugar de enviar una carga de bloques no válida.

## Aprobaciones de exec en Slack

Slack puede actuar como un cliente nativo de aprobación con botones interactivos e interacciones, en lugar de volver a la interfaz web o al terminal.

- Las aprobaciones de exec usan `channels.slack.execApprovals.*` para el enrutamiento nativo en MD/canales.
...
La configuración nativa explícita de Slack solo es necesaria cuando quieres sobrescribir aprobadores, agregar filtros o
...
- Las ediciones/eliminaciones de mensajes y las difusiones de hilos se mapean a eventos del sistema.
- Los eventos de agregar/quitar reacciones se mapean a eventos del sistema.
- Los eventos de entrada/salida de miembros, creación/cambio de nombre de canales y agregar/quitar fijados se mapean a eventos del sistema.
...
- El iniciador del hilo y la siembra inicial del contexto del historial del hilo se filtran mediante listas de remitentes permitidos configuradas cuando corresponde.
- Las acciones de bloques e interacciones modales emiten eventos estructurados del sistema `Slack interaction: ...` con campos de carga útiles enriquecidos:
...
- [Referencia de configuración - Slack](/es/gateway/configuration-reference#slack)
  ...
  - interruptor de compatibilidad: `dangerouslyAllowNameMatching` (de emergencia; mantenlo desactivado salvo que sea necesario)
...
```
  ...
  <Accordion title="Los mensajes de MD se ignoran">
    Comprueba:
...
```
  ...
  <Accordion title="Socket mode no se conecta">
    Valida los tokens de bot y de aplicación, y la activación de Socket Mode en la configuración de la aplicación de Slack.
  ...
  <Accordion title="El modo HTTP no recibe eventos">
    ...
    - signing secret
    - ruta del webhook
    - URL de solicitudes de Slack (Eventos + Interactividad + Comandos slash)
    - `webhookPath` único por cuenta HTTP

    Si `signingSecretStatus: "configured_unavailable"` aparece en las
    instantáneas de cuentas, la cuenta HTTP está configurada pero el tiempo de ejecución actual no pudo
