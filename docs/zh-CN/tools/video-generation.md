...
title: 视频生成
x-i18n:
    generated_at: "2026-04-06T01:23:42Z"
    model: gpt-5.4
    provider: openai
...
当任务正在进行时，在同一会话中重复调用 `video_generate` 不会启动新的生成，而是返回当前任务状态。使用 `openclaw tasks list` 或 `openclaw tasks show <taskId>` 可从 CLI 检查进度。
...
| BytePlus（国际版） | `seedance-1-0-lite-t2v-250428`  | 是   | 1 张图像         | 否               | `BYTEPLUS_API_KEY`                       |
...
| `durationSeconds` | number  | 目标时长（秒，四舍五入为最接近的提供商支持值）                           |
...
并非所有提供商都支持所有参数。不受支持的覆盖项会在尽力而为的基础上被忽略，并在工具结果中报告为警告。硬性能力限制（例如参考输入过多）会在提交前直接失败。

## 操作

- **generate**（默认）-- 根据给定提示和可选参考输入创建视频。
- **status** -- 检查当前会话中进行中的视频任务状态，而不启动新的生成。
...
4. **自动检测** -- 使用具有有效认证的提供商，先从当前默认提供商开始，再按字母顺序尝试其余提供商。
...
| Runway   | 通过 data URI 支持本地文件。视频转视频需要 `runway/gen4_aleph`。纯文本运行提供 `16:9` 和 `9:16` 宽高比。                               |
| Together | 仅支持单张参考图像。                                                                                                                     |
| Vydra    | 直接使用 `https://www.vydra.ai/api/v1` 以避免认证在重定向过程中丢失。`veo3` 仅作为文生视频内置；`kling` 需要远程图像 URL。              |
...
- [BytePlus（国际版）](/providers/byteplus)
