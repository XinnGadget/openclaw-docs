...
- 原生斜杠命令在隔离的命令会话中运行（`agent:<agentId>:discord:slash:<userId>`），同时仍会把 `CommandTargetSessionKey` 传递给被路由的对话会话。

## 论坛频道

Discord 论坛和媒体频道只接受主题帖。OpenClaw 支持两种创建方式：

- 向论坛父级（`channel:<forumId>`）发送消息以自动创建主题。主题标题使用你消息中的第一条非空行。
...
```
  ...
  <Accordion title="在 Discord 中进行审批">
