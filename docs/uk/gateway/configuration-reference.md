...
x-i18n:
    generated_at: "2026-04-06T00:57:07Z"
    model: gpt-5.4
    provider: openai
...
| `allowlist`         | Лише відправники в `allowFrom` (або у сховищі дозволів після прив'язки) |
...
| `open`                | Обійти allowlist груп (фільтрація за згадками все ще застосовується) |
...
Коди прив'язки закінчують дію через 1 годину. Кількість очікуваних запитів на DM-прив'язку обмежена **3 на канал**.
...
- Вихідні команди типово використовують обліковий запис `default`, якщо він існує; інакше — перший налаштований ID облікового запису (у відсортованому порядку).
...
- Застарілий auth dir для одного облікового запису Baileys мігрується командою `openclaw doctor` до `whatsapp/default`.
...
```json5
{
  channels: {
    telegram: {
      ...
      linkPreview: true,
      streaming: "partial", // off | partial | block | progress (типово: off; явно ввімкніть, щоб уникнути обмежень rate limit для preview-edit)
...
- У конфігураціях з кількома обліковими записами (2+ ID облікових записів) встановіть явний обліковий запис за замовчуванням (`channels.telegram.defaultAccount` або `channels.telegram.accounts.default`), щоб уникнути резервної маршрутизації; `openclaw doctor` попереджає, якщо цього немає або значення недійсне.
- `configWrites: false` блокує записи конфігурації, ініційовані з Telegram (міграції ID supergroup, `/config set|unset`).
- Записи верхнього рівня `bindings[]` з `type: "acp"` налаштовують постійні ACP-прив'язки для тем форумів (використовуйте канонічний `chatId:topic:topicId` у `match.peer.id`). Семантика полів спільна з [ACP Agents](/uk/tools/acp-agents#channel-specific-settings).
- Прев'ю потоків Telegram використовують `sendMessage` + `editMessageText` (працює в особистих і групових чатах).
...
- Токен: `channels.discord.token`, з `DISCORD_BOT_TOKEN` як резервним значенням для облікового запису за замовчуванням.
- Прямі вихідні виклики, які надають явний Discord `token`, використовують цей токен для виклику; налаштування retry/політик облікового запису все одно беруться з вибраного облікового запису в активному runtime snapshot.
...
- Slug guild — у нижньому регістрі з пробілами, заміненими на `-`; ключі каналів використовують slug-ім'я (без `#`). Віддавайте перевагу ID guild.
...
- `channels.discord.threadBindings` керує маршрутизацією, прив'язаною до гілок Discord:
  - `enabled`: перевизначення Discord для функцій сесій, прив'язаних до гілки (`/focus`, `/unfocus`, `/agents`, `/session idle`, `/session max-age`, а також прив'язана доставка/маршрутизація)
  - `idleHours`: перевизначення Discord для авто-скасування фокуса через неактивність у годинах (`0` вимикає)
  - `maxAgeHours`: перевизначення Discord для жорсткого максимального віку в годинах (`0` вимикає)
  - `spawnSubagentSessions`: opt-in перемикач для автостворення/прив'язки гілок `sessions_spawn({ thread: true })`
- Записи верхнього рівня `bindings[]` з `type: "acp"` налаштовують постійні ACP-прив'язки для каналів і гілок (використовуйте channel/thread id у `match.peer.id`). Семантика полів спільна з [ACP Agents](/uk/tools/acp-agents#channel-specific-settings).
...
- OpenClaw додатково намагається відновити прийом голосу, виходячи та повторно входячи у голосову сесію після повторних збоїв дешифрування.
...
- `channels.discord.dangerouslyAllowNameMatching` знову вмикає зіставлення за змінним іменем/тегом (режим сумісності break-glass).
- `channels.discord.execApprovals`: нативна доставка схвалень exec у Discord і авторизація схвалювачів.
  ...
  - `target`: куди надсилати запити на схвалення. `"dm"` (типово) надсилає в DM схвалювачам, `"channel"` — в початковий канал, `"both"` — в обидва місця. Коли ціль включає `"channel"`, кнопками можуть користуватися лише визначені схвалювачі.
  - `cleanupAfterResolve`: якщо `true`, видаляє DM зі схваленням після схвалення, відмови або тайм-ауту.

**Режими сповіщень про реакції:** `off` (немає), `own` (повідомлення бота, типово), `all` (усі повідомлення), `allowlist` (від `guilds.<id>.users` на всіх повідомленнях).
...
- `channels.googlechat.dangerouslyAllowNameMatching` знову вмикає зіставлення за змінною email principal (режим сумісності break-glass).
...
```json5
{
  channels: {
    slack: {
      ...
      nativeStreaming: true, // використовувати нативний streaming API Slack, коли streaming=partial
      mediaMaxMb: 20,
      execApprovals: {
        enabled: "auto", // true | false | "auto"
        approvers: ["U123"],
...
- Знімки облікових записів Slack показують поля джерела/стану для кожного облікового запису, такі як
...
**Ізоляція сесій гілок:** `thread.historyScope` — для кожної гілки окремо (типово) або спільний для каналу. `thread.inheritParent` копіює стенограму батьківського каналу в нові гілки.
...
- `channels.slack.execApprovals`: нативна доставка схвалень exec у Slack і авторизація схвалювачів. Та сама схема, що й у Discord: `enabled` (`true`/`false`/`"auto"`), `approvers` (ID користувачів Slack), `agentFilter`, `sessionFilter` і `target` (`"dm"`, `"channel"` або `"both"`).
  ...
  channels: {
    mattermost: {
      ...
      commands: {
        ...
        // Необов'язкова явна URL для розгортань через reverse proxy/public
...
- Нативні зворотні виклики slash-аутентифікуються окремими токенами команд,
  які Mattermost повертає під час реєстрації slash command. Якщо реєстрація не вдається або жодна
  команда не активована, OpenClaw відхиляє callback з
  `Unauthorized: invalid command token.`
- Для приватних/tailnet/internal callback-хостів Mattermost може вимагати, щоб
...
- `channels.mattermost.groups.<channelId>.requireMention`: перевизначення фільтрації за згадками для конкретного каналу (`"*"` для типового значення).
...
- Записи верхнього рівня `bindings[]` з `type: "acp"` можуть прив'язувати розмови BlueBubbles до постійних ACP-сесій. Використовуйте BlueBubbles handle або target string (`chat_id:*`, `chat_guid:*`, `chat_identifier:*`) у `match.peer.id`. Спільна семантика полів: [ACP Agents](/uk/tools/acp-agents#channel-specific-settings).
...
- SCP використовує сувору перевірку ключів хоста, тож переконайтеся, що ключ relay host уже існує в `~/.ssh/known_hosts`.
...
Matrix має бекенд-розширення і налаштовується в `channels.matrix`.
...
- `channels.matrix.execApprovals`: нативна доставка схвалень exec у Matrix і авторизація схвалювачів.
  - `enabled`: `true`, `false` або `"auto"` (типово). В auto mode схвалення exec активуються, коли схвалювачів можна визначити з `approvers` або `commands.ownerAllowFrom`.
...
- `channels.matrix.dm.sessionScope` керує тим, як DM у Matrix групуються в сесії: `per-user` (типово) спільно використовує сесію за маршрутизованим peer, а `per-room` ізолює кожну DM-кімнату.
- Matrix status probes і живі пошуки каталогу використовують ту саму політику proxy, що й трафік runtime.
- Повна конфігурація Matrix, правила адресації та приклади налаштування задокументовані в [Matrix](/uk/channels/matrix).

### Microsoft Teams

Microsoft Teams має бекенд-розширення і налаштовується в `channels.msteams`.
...
IRC має бекенд-розширення і налаштовується в `channels.irc`.
...
Запускайте кілька облікових записів на канал (кожен має власний `accountId`):
...
- Якщо ви додаєте недефолтний обліковий запис через `openclaw channels add` (або onboarding каналу), поки все ще використовуєте однoоблікову конфігурацію каналу верхнього рівня, OpenClaw спочатку переносить account-scoped значення верхнього рівня для одного облікового запису до мапи облікових записів каналу, щоб початковий обліковий запис продовжив працювати. Більшість каналів переміщують їх до `channels.<channel>.accounts.default`; Matrix натомість може зберегти наявну відповідну іменовану/default ціль.
- Наявні прив'язки лише для каналу (без `accountId`) і надалі відповідатимуть обліковому запису за замовчуванням; прив'язки на рівні облікового запису залишаються необов'язковими.
- `openclaw doctor --fix` також виправляє змішані форми, переміщуючи account-scoped значення верхнього рівня для одного облікового запису до promoted account, вибраного для цього каналу. Більшість каналів використовують `accounts.default`; Matrix може зберегти наявну відповідну іменовану/default ціль.
...
### Фільтрація групових чатів за згадками

Повідомлення в групах типово **вимагають згадки** (метадані згадки або безпечні regex-шаблони). Це стосується групових чатів WhatsApp, Telegram, Discord, Google Chat та iMessage.
...
- Фільтрація за згадками застосовується лише тоді, коли виявлення можливе (нативні згадки або хоча б один шаблон).
...
- `config: true` вмикає `/config` (читання/запис `openclaw.json`). Для клієнтів gateway `chat.send` постійні записи `/config set|unset` також потребують `operator.admin`; read-only `/config show` залишається доступним для звичайних операторських клієнтів з правом запису.
...
- `allowFrom` задається для кожного постачальника. Якщо встановлено, це **єдине** джерело авторизації (allowlist каналу/прив'язка і `useAccessGroups` ігноруються).
...
Необов'язковий корінь репозиторію, який показується в рядку Runtime системного prompt. Якщо не встановлено, OpenClaw автоматично визначає його, піднімаючись вгору від workspace.
...
Максимальна кількість символів на bootstrap-файл workspace перед обрізанням. Типово: `20000`.
...
Максимальна загальна кількість символів, які вставляються з усіх bootstrap-файлів workspace. Типово: `150000`.
...
Максимальний розмір у пікселях для довшої сторони зображення в блоках transcript/tool image перед викликами постачальника.
Типово: `1200`.

Менші значення зазвичай зменшують використання vision-токенів і розмір payload запиту для сценаріїв із великою кількістю скріншотів.
...
- `model`: приймає або рядок (`"provider/model"`), або об'єкт (`{ primary, fallbacks }`).
  - Форма рядка встановлює лише primary model.
  - Форма об'єкта встановлює primary плюс упорядковані failover-моделі.
- `imageModel`: приймає або рядок (`"provider/model"`), або об'єкт (`{ primary, fallbacks }`).
  - Використовується шляхом інструмента `image` як його конфігурація vision model.
  - Також використовується для резервної маршрутизації, коли вибрана/типова модель не може приймати зображення.
- `imageGenerationModel`: приймає або рядок (`"provider/model"`), або об'єкт (`{ primary, fallbacks }`).
  ...
  - Якщо ви напряму вибираєте provider/model, також налаштуйте відповідну auth/API key постачальника (наприклад `GEMINI_API_KEY` або `GOOGLE_API_KEY` для `google/*`, `OPENAI_API_KEY` для `openai/*`, `FAL_KEY` для `fal/*`).
  - Якщо пропущено, `image_generate` усе одно може вивести типового постачальника з наявною auth. Спочатку він пробує поточного типового постачальника, потім інших зареєстрованих постачальників генерації зображень у порядку provider-id.
- `musicGenerationModel`: приймає або рядок (`"provider/model"`), або об'єкт (`{ primary, fallbacks }`).
  ...
  - Якщо пропущено, `music_generate` усе одно може вивести типового постачальника з наявною auth. Спочатку він пробує поточного типового постачальника, потім інших зареєстрованих постачальників генерації музики в порядку provider-id.
  - Якщо ви напряму вибираєте provider/model, також налаштуйте відповідну auth/API key постачальника.
- `videoGenerationModel`: приймає або рядок (`"provider/model"`), або об'єкт (`{ primary, fallbacks }`).
  - Використовується спільною можливістю генерації відео та вбудованим інструментом `video_generate`.
  - Типові значення: `qwen/wan2.6-t2v`, `qwen/wan2.6-i2v`, `qwen/wan2.6-r2v`, `qwen/wan2.6-r2v-flash` або `qwen/wan2.7-r2v`.
  - Якщо пропущено, `video_generate` усе одно може вивести типового постачальника з наявною auth. Спочатку він пробує поточного типового постачальника, потім інших зареєстрованих постачальників генерації відео в порядку provider-id.
  - Якщо ви напряму вибираєте provider/model, також налаштуйте відповідну auth/API key постачальника.
  - Вбудований постачальник генерації відео Qwen наразі підтримує до 1 вихідного відео, 1 вхідного зображення, 4 вхідних відео, тривалість 10 секунд і параметри рівня постачальника `size`, `aspectRatio`, `resolution`, `audio` і `watermark`.
...
- `model.primary`: формат `provider/model` (наприклад `openai/gpt-5.4`). Якщо ви пропустите provider, OpenClaw спочатку спробує alias, потім унікальний configured-provider match для точного model id і лише потім повернеться до configured default provider (застаріла поведінка сумісності, тож краще явно вказувати `provider/model`). Якщо цей provider більше не надає налаштовану типову модель, OpenClaw повертається до першого налаштованого provider/model замість того, щоб показувати застарілу типову модель видаленого provider.
...
- `params`: глобальні типові параметри постачальника, які застосовуються до всіх моделей. Установлюються в `agents.defaults.params` (наприклад `{ cacheRetention: "long" }`).
- Пріоритет об'єднання `params` (конфігурація): `agents.defaults.params` (глобальна база) перевизначається `agents.defaults.models["provider/model"].params` (для моделі), потім `agents.list[].params` (для відповідного ID агента) перевизначає по ключах. Деталі див. у [Prompt Caching](/uk/reference/prompt-caching).
- Засоби запису конфігурації, що змінюють ці поля (наприклад `/models set`, `/models set-image` і команди add/remove fallback), зберігають канонічну форму об'єкта і зберігають наявні списки fallback, коли це можливо.
...
- `every`: рядок тривалості (ms/s/m/h). Типово: `30m` (auth через API key) або `1h` (OAuth auth). Установіть `0m`, щоб вимкнути.
- `suppressToolErrorWarnings`: якщо true, пригнічує payload попереджень про помилки інструментів під час запусків heartbeat.
- `directPolicy`: політика прямої/DM-доставки. `allow` (типово) дозволяє доставку в direct-target. `block` пригнічує доставку в direct-target і генерує `reason=dm-blocked`.
...
- `isolatedSession`: якщо true, кожен heartbeat запускається в новій сесії без попередньої історії розмови. Такий самий шаблон ізоляції, як і в cron `sessionTarget: "isolated"`. Зменшує вартість у токенах на heartbeat приблизно зі ~100K до ~2-5K токенів.
- Для агента: задайте `agents.list[].heartbeat`. Якщо будь-який агент визначає `heartbeat`, heartbeat запускатимуть **лише ці агенти**.
- Heartbeat виконують повні ходи агента — менші інтервали спалюють більше токенів.
...
- `timeoutSeconds`: максимальна кількість секунд для однієї операції compaction, після якої OpenClaw її перериває. Типово: `900`.
...
- `postCompactionSections`: необов'язкові назви секцій H2/H3 з AGENTS.md, які потрібно повторно вставити після compaction. Типово `["Session Startup", "Red Lines"]`; установіть `[]`, щоб вимкнути повторне вставлення. Якщо не встановлено або явно задано цю типову пару, старі заголовки `Every Session`/`Safety` також приймаються як застарілий резервний варіант.
- `model`: необов'язкове перевизначення `provider/model-id` лише для compaction summarization. Використовуйте це, коли основна сесія має залишатися на одній моделі, а compaction summary — виконуватися на іншій; якщо не встановлено, compaction використовує primary model сесії.
- `notifyUser`: коли `true`, надсилає коротке повідомлення користувачу, коли починається compaction (наприклад, "Compacting context..."). Типово вимкнено, щоб compaction залишався безшумним.
...
- `ttl` керує тим, як часто обрізання може виконуватися знову (після останнього дотику до кешу).
- Обрізання спочатку м'яко скорочує завеликі результати інструментів, а потім за потреби жорстко очищає старіші результати інструментів.
...
- Перевизначення для каналів: `channels.<channel>.blockStreamingCoalesce` (і варіанти на рівні облікового запису). Для Signal/Slack/Discord/Google Chat типово `minChars: 1500`.
- `humanDelay`: випадкова пауза між block replies. `natural` = 800–2500ms. Перевизначення для агента: `agents.list[].humanDelay`.

Деталі поведінки та chunking див. у [Streaming](/uk/concepts/streaming).
...
- `target`: SSH target у формі `user@host[:port]`
- `command`: команда SSH client (типово: `ssh`)
- `workspaceRoot`: абсолютний віддалений root, що використовується для workspace за scope
...
- Значення `*Data` на основі SecretRef визначаються з активного secrets runtime snapshot перед запуском sandbox session

**Поведінка SSH backend:**

- ініціалізує віддалений workspace один раз після create або recreate
...
- `none`: workspace sandbox за scope в `~/.openclaw/sandboxes`
...
- `remote`: ініціалізувати віддалений один раз, коли створюється sandbox, а потім зберігати віддалений workspace канонічним
...
**`setupCommand`** виконується один раз після створення контейнера (через `sh -lc`). Потребує вихідного доступу до мережі, доступного для запису root і користувача root.

**Типово контейнери використовують `network: "none"`** — установіть `"bridge"` (або власну bridge network), якщо агенту потрібен вихідний доступ.
...
**Sandboxed browser** (`sandbox.browser.enabled`): Chromium + CDP у контейнері. URL noVNC вставляється в системний prompt. Не потребує `browser.enabled` в `openclaw.json`.
Доступ спостерігача noVNC типово використовує VNC auth, і OpenClaw генерує URL із короткоживучим токеном (замість відкриття пароля в спільній URL).

- `allowHostControl: false` (типово) блокує націлювання sandboxed sessions на браузер хоста.
- `network` типово дорівнює `openclaw-sandbox-browser` (окрема bridge network). Установлюйте `bridge` лише тоді, коли вам явно потрібна глобальна bridge connectivity.
- `cdpSourceRange` необов'язково обмежує CDP ingress на межі контейнера до CIDR-діапазону (наприклад `172.21.0.1/32`).
- `sandbox.browser.binds` монтує додаткові каталоги хоста лише в контейнер sandbox browser. Якщо задано (включно з `[]`), це замінює `docker.binds` для контейнера browser.
- Типові параметри запуску визначені в `scripts/sandbox-browser-entrypoint.sh` і налаштовані для контейнерних хостів:
  ...
  - `--disable-3d-apis`, `--disable-software-rasterizer` і `--disable-gpu`
    типово увімкнені, і їх можна вимкнути через
  ...
  - Типові значення є базовими для образу контейнера; щоб змінити їх, використовуйте
    власний browser image з власним entrypoint.

</Accordion>

Sandboxing браузера і `sandbox.docker.binds` наразі доступні лише для Docker.

Збирання образів:

```bash
scripts/sandbox-setup.sh           # головний sandbox image
...
- `default`: якщо встановлено для кількох, перший має пріоритет (логуються попередження). Якщо не встановлено ніде, типовим є перший елемент списку.
- `model`: форма рядка перевизначає лише `primary`; форма об'єкта `{ primary, fallbacks }` перевизначає обидва (`[]` вимикає глобальні fallback). Cron jobs, які перевизначають лише `primary`, усе ще успадковують типові fallback, якщо ви не встановите `fallbacks: []`.
- `params`: stream params для окремого агента, що об'єднуються поверх вибраного запису моделі з `agents.defaults.models`. Використовуйте це для перевизначень, специфічних для агента, як-от `cacheRetention`, `temperature` або `maxTokens`, не дублюючи весь каталог моделей.
- `skills`: необов'язковий allowlist Skills для окремого агента. Якщо пропущено, агент успадковує `agents.defaults.skills`, якщо вони задані; явний список замінює типові значення, а `[]` означає відсутність Skills.
- `thinkingDefault`: необов'язковий типовий рівень thinking для агента (`off | minimal | low | medium | high | xhigh | adaptive`). Перевизначає `agents.defaults.thinkingDefault` для цього агента, якщо не задано перевизначення для повідомлення або сесії.
- `reasoningDefault`: необов'язкове типове значення видимості reasoning для агента (`on | off | stream`). Застосовується, якщо не задано перевизначення reasoning для повідомлення або сесії.
- `fastModeDefault`: необов'язкове типове значення fast mode для агента (`true | false`). Застосовується, якщо не задано перевизначення fast-mode для повідомлення або сесії.
- `runtime`: необов'язковий runtime descriptor для агента. Використовуйте `type: "acp"` із типовими значеннями `runtime.acp` (`agent`, `backend`, `mode`, `cwd`), коли агент має типово використовувати ACP harness sessions.
...
- Захист успадкування sandbox: якщо сесія-запитувач знаходиться в sandbox, `sessions_spawn` відхиляє цілі, які виконувалися б без sandbox.
...
Для записів `type: "acp"` OpenClaw визначає відповідність за точною ідентичністю розмови (`match.channel` + account + `match.peer.id`) і не використовує порядок рівнів route binding, наведений вище.
...
```json5
{
  session: {
    ...
    maintenance: {
      ...
      highWaterBytes: "400mb", // необов'язкова ціль після очищення
    },
    threadBindings: {
      enabled: true,
      idleHours: 24, // типове авто-скасування фокуса через неактивність у годинах (`0` вимикає)
...
- **`identityLinks`**: зіставлення канонічних ID з peer із префіксами постачальників для спільного використання сесії між каналами.
- **`reset`**: основна політика reset. `daily` скидає о `atHour` за місцевим часом; `idle` скидає після `idleMinutes`. Якщо налаштовано обидва, перемагає те, що сплине раніше.
- **`resetByType`**: перевизначення для типу (`direct`, `group`, `thread`). Застарілий `dm` приймається як alias для `direct`.
- **`parentForkMaxTokens`**: максимальний `totalTokens` батьківської сесії, дозволений для створення forked thread session (типово `100000`).
  - Якщо батьківський `totalTokens` перевищує це значення, OpenClaw запускає нову thread session замість успадкування історії transcript батьківської сесії.
  - Установіть `0`, щоб вимкнути цей захист і завжди дозволяти parent forking.
- **`mainKey`**: застаріле поле. Runtime тепер завжди використовує `"main"` для основного кошика direct-chat.
- **`agentToAgent.maxPingPongTurns`**: максимальна кількість ходів reply-back між агентами під час agent-to-agent обміну (ціле число, діапазон: `0`–`5`). `0` вимикає ping-pong chaining.
- **`sendPolicy`**: match за `channel`, `chatType` (`direct|group|channel`, із застарілим alias `dm`), `keyPrefix` або `rawKeyPrefix`. Перший deny перемагає.
- **`maintenance`**: елементи керування очищенням і retention сховища сесій.
  ...
  - `pruneAfter`: віковий cutoff для застарілих записів (типово `30d`).
  ...
  - `maxDiskBytes`: необов'язковий бюджет диска для каталогу сесій. У режимі `warn` логуються попередження; у режимі `enforce` спочатку видаляються найстаріші артефакти/сесії.
...
- **`threadBindings`**: глобальні типові значення для функцій сесій, прив'язаних до гілок.
  ...
  - `idleHours`: типове авто-скасування фокуса через неактивність у годинах (`0` вимикає; постачальники можуть перевизначати)
...
Порядок визначення (перемагає найбільш специфічне): account → channel → global. `""` вимикає і зупиняє каскад. `"auto"` виводить `[{identity.name}]`.
...
- Порядок визначення: account → channel → `messages.ackReaction` → резервне значення identity.
...
- `messages.statusReactions.enabled`: вмикає реакції життєвого циклу статусу в Slack, Discord і Telegram.
  У Slack і Discord, якщо не встановлено, status reactions залишаються ввімкненими, коли активні ack reactions.
  У Telegram установіть це явно в `true`, щоб увімкнути lifecycle status reactions.

### Вхідний debounce

Об'єднує швидкі текстові повідомлення від одного відправника в один хід агента. Media/вкладення скидаються негайно. Керівні команди обходять debounce.
...
- `modelOverrides` типово ввімкнено; `modelOverrides.allowProvider` типово дорівнює `false` (opt-in).
- API keys використовують `ELEVENLABS_API_KEY`/`XI_API_KEY` і `OPENAI_API_KEY` як резервні значення.
...
- Коли `openai.baseUrl` вказує на endpoint, що не належить OpenAI, OpenClaw вважає його OpenAI-compatible TTS server і послаблює перевірку model/voice.
...
- Застарілі плоскі ключі Talk (`talk.voiceId`, `talk.voiceAliases`, `talk.modelId`, `talk.outputFormat`, `talk.apiKey`) слугують лише для сумісності й автоматично мігруються до `talk.providers.<provider>`.
- Voice ID використовують `ELEVENLABS_VOICE_ID` або `SAG_VOICE_ID` як резервні значення.
...
- `silenceTimeoutMs` керує тим, скільки режим Talk чекає після тиші користувача, перш ніж надіслати transcript. Якщо не задано, зберігається типове вікно паузи платформи (`700 ms на macOS і Android, 900 ms на iOS`).
...
Під час локального onboarding нові локальні конфігурації типово отримують `tools.profile: "coding"`, якщо це не задано (наявні явні профілі зберігаються).
...
| `full`      | Без обмежень (так само, як і не встановлено)                                                                                   |
...
| `group:openclaw`   | Усі вбудовані інструменти (виключає provider plugins)                                                                      |
...
Додатково обмежує інструменти для конкретних постачальників або моделей. Порядок: base profile → provider profile → allow/deny.
...
- `/elevated on|off|ask|full` зберігає стан для сесії; inline directives застосовуються лише до одного повідомлення.
- Підвищений `exec` обходить sandboxing і використовує налаштований escape path (`gateway` типово або `node`, коли ціль exec — `node`).
...
Налаштування можна визначати глобально в `tools.loopDetection` і перевизначати для агента в `agents.list[].tools.loopDetection`.
...
- `detectors.pingPong`: попереджати/блокувати почергові шаблони пар без прогресу.
...
- Після помилок використовується наступний запис.

Auth постачальника дотримується стандартного порядку: `auth-profiles.json` → env vars → `models.providers.*.apiKey`.
...
Типово: `tree` (поточна сесія + сесії, породжені нею, як-от subagents).
...
- `agent`: будь-яка сесія, що належить поточному ID агента (може включати інших користувачів, якщо ви запускаєте per-sender sessions під тим самим ID агента).
- `all`: будь-яка сесія. Націлювання між агентами все одно потребує `tools.agentToAgent`.
- Затиск sandbox: коли поточна сесія знаходиться в sandbox і `agents.defaults.sandbox.sessionToolsVisibility="spawned"`, visibility примусово встановлюється в `tree`, навіть якщо `tools.sessions.visibility="all"`.
...
- Base64-входи проходять перевірку строгого алфавіту/відступів і попередній захист розміру до декодування.
...
Експериментальні прапори вбудованих інструментів. Типово вимкнено, якщо не застосовується правило автоматичного ввімкнення, специфічне для runtime.
...
- Типово: `false` для постачальників, відмінних від OpenAI. Запуски OpenAI і OpenAI Codex вмикають його автоматично.
- Коли увімкнено, системний prompt також додає вказівки з використання, щоб модель застосовувала його лише для суттєвої роботи і тримала не більше одного кроку в стані `in_progress`.
...
- Використовуйте `authHeader: true` + `headers` для користувацьких потреб auth.
- Перевизначте корінь конфігурації агента через `OPENCLAW_AGENT_DIR` (або `PI_CODING_AGENT_DIR`, застарілий alias env variable).
- Пріоритет об'єднання для відповідних ID постачальників:
  ...
  - Значення `apiKey` постачальника, керовані через SecretRef, оновлюються з source markers (`ENV_VAR_NAME` для env refs, `secretref-managed` для file/exec refs) замість збереження визначених секретів.
  - Значення заголовків постачальника, керовані через SecretRef, оновлюються з source markers (`secretref-env:ENV_VAR_NAME` для env refs, `secretref-managed` для file/exec refs).
  - Порожні або відсутні agent `apiKey`/`baseUrl` повертаються до `models.providers` у конфігурації.
  - Відповідні `contextWindow`/`maxTokens` моделі використовують більше значення між явною конфігурацією і неявними значеннями каталогу.
  - Відповідний `contextTokens` моделі зберігає явний runtime cap, якщо його задано; використовуйте його, щоб обмежити ефективний контекст без зміни нативних метаданих моделі.
  - Використовуйте `models.mode: "replace"`, якщо хочете, щоб конфігурація повністю переписала `models.json`.
  - Збереження marker є авторитетним щодо джерела: markers записуються з активного snapshot конфігурації джерела (до визначення), а не з визначених runtime secret values.
...
- `models.providers.*.apiKey`: облікові дані постачальника (бажано SecretRef/env substitution).
...
- `models.providers.*.models.*.contextTokens`: необов'язкова runtime cap для контексту. Використовуйте це, коли вам потрібен менший ефективний бюджет контексту, ніж нативний `contextWindow` моделі.
...
- `plugins.entries.amazon-bedrock.config.discovery.providerFilter`: необов'язковий provider-id filter для цільового discovery.
...
- `plugins.entries.amazon-bedrock.config.discovery.defaultMaxTokens`: резервна максимальна кількість output tokens для виявлених моделей.
...
Нативні endpoint Moonshot рекламують сумісність використання streaming на спільному
...
На шляху streaming, сумісному з Anthropic, OpenClaw типово вимикає thinking MiniMax,
...
Див. [Local Models](/uk/gateway/local-models). Коротко: запускайте велику локальну модель через LM Studio Responses API на серйозному обладнанні; залишайте хостингові моделі злитими для fallback.
...
- `install.preferBrew`: коли true, надавати перевагу інсталяторам Homebrew, якщо `brew`
  доступний, а вже потім переходити до інших типів інсталяторів.
- `install.nodeManager`: пріоритет node installer для специфікацій `metadata.openclaw.install`
  (`npm` | `pnpm` | `yarn` | `bun`).
- `entries.<skillKey>.enabled: false` вимикає skill, навіть якщо він bundled/installed.
- `entries.<skillKey>.apiKey`: зручне поле API key для skills, що оголошують основну env variable (звичайний рядок або об'єкт SecretRef).
...
- Discovery приймає нативні plugins OpenClaw, а також сумісні bundled Codex і Claude, включно з manifestless bundled Claude у layout за замовчуванням.
- **Зміни конфігурації потребують перезапуску gateway.**
- `allow`: необов'язковий allowlist (завантажуються лише перелічені plugins). `deny` має пріоритет.
- `plugins.entries.<id>.apiKey`: зручне поле API key на рівні plugin (коли plugin його підтримує).
- `plugins.entries.<id>.env`: мапа env variables для plugin.
- `plugins.entries.<id>.hooks.allowPromptInjection`: коли `false`, ядро блокує `before_prompt_build` та ігнорує поля, що змінюють prompt, із застарілого `before_agent_start`, зберігаючи застарілі `modelOverride` і `providerOverride`. Застосовується до нативних plugin hooks і підтримуваних каталогів hooks, наданих bundle.
- `plugins.entries.<id>.subagent.allowModelOverride`: явно довірити цьому plugin право запитувати перевизначення `provider` і `model` для окремих запусків background subagent.
...
- `plugins.entries.firecrawl.config.webFetch`: налаштування постачальника web-fetch Firecrawl.
  - `apiKey`: API key Firecrawl (приймає SecretRef). Використовує резервно `plugins.entries.firecrawl.config.webSearch.apiKey`, застарілий `tools.web.fetch.firecrawl.apiKey` або env var `FIRECRAWL_API_KEY`.
  - `baseUrl`: base URL API Firecrawl (типово: `https://api.firecrawl.dev`).
  - `onlyMainContent`: вилучати лише основний вміст зі сторінок (типово: `true`).
...
- `plugins.entries.memory-core.config.dreaming`: налаштування memory dreaming (експериментально). Фази та пороги див. у [Dreaming](/uk/concepts/dreaming).
  ...
  - `frequency`: cron cadence для кожного повного проходу dreaming (`"0 3 * * *"` типово).
  - політика фаз і пороги — це деталі реалізації (не користувацькі ключі конфігурації).
- Увімкнені bundled Claude plugins також можуть додавати вбудовані типові налаштування Pi з `settings.json`; OpenClaw застосовує їх як очищені налаштування агента, а не як сирі патчі конфігурації OpenClaw.
- `plugins.slots.memory`: вибрати активний ID plugin memory або `"none"`, щоб вимкнути plugins memory.
- `plugins.slots.contextEngine`: вибрати активний ID plugin context engine; типово `"legacy"`, доки ви не встановите і не виберете інший engine.
- `plugins.installs`: метадані інст