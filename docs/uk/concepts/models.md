...
title: CLI моделей
x-i18n:
    generated_at: "2026-04-06T00:52:28Z"
    model: gpt-5.4
    provider: openai
...
Див. [/concepts/model-failover](/uk/concepts/model-failover) щодо ротації профілів
автентифікації, періодів охолодження та того, як це взаємодіє з резервними варіантами.
...
- `agents.defaults.pdfModel` використовується інструментом `pdf`. Якщо його не вказано, інструмент
  переходить до `agents.defaults.imageModel`, а потім до підсумково визначеної моделі сесії/типової моделі.
...
- Типові налаштування для окремих агентів можуть перевизначати `agents.defaults.model` через `agents.list[].model` разом із прив’язками (див. [/concepts/multi-agent](/uk/concepts/multi-agent)).
...
ніби на повідомлення «не відповіли». Виправлення: або
...
- Якщо ви не вказуєте провайдера, OpenClaw визначає введення в такому порядку:
  ...
  3. застарілий резервний перехід до налаштованого типового провайдера
     ...
     показувати застарілий типовий варіант від видаленого провайдера.
...
Перевірка потребує OpenRouter API key (із профілів автентифікації або
...
Збереження маркерів є авторитетним щодо джерела: OpenClaw записує маркери з активного знімка конфігурації джерела (до підсумкового визначення), а не з підсумково визначених значень секретів часу виконання.
Це застосовується щоразу, коли OpenClaw повторно генерує `models.json`, зокрема для шляхів, керованих командами, як-от `openclaw agent`.
