---
read_when:
    - 设置 Slack 或调试 Slack 的 socket/HTTP 模式
summary: Slack 设置与运行时行为（Socket Mode + HTTP Request URL）
title: Slack
x-i18n:
    generated_at: "2026-04-06T06:59:07Z"
    model: gpt-5.4
    provider: openai
...
状态：已可在生产环境中用于通过 Slack 应用集成实现私信 + 渠道。默认模式为 Socket Mode；也支持 HTTP Request URL。
  ...
  <Tab title="Socket Mode（默认）">
    <Steps>
      <Step title="创建 Slack 应用和令牌">
        ...
        - 启用 **Socket Mode**
        - 创建具有 `connections:write` 权限的 **App Token**（`xapp-...`）
  ...
  <Tab title="HTTP Request URL">
    <Steps>
      <Step title="为 HTTP 配置 Slack 应用">
        ...
        - 将 Event Subscriptions、Interactivity 和 Slash command 的 Request URL 设为相同的 webhook 路径（默认 `/slack/events`）
...
```
      ...
      <Step title="为多账户 HTTP 使用唯一的 webhook 路径">
        ...
        为每个账户分配不同的 `webhookPath`，以避免注册冲突。
      </Step>
    </Steps>
...
- `userToken`（`xoxp-...`）仅支持在配置中设置（无环境变量回退），并且默认启用只读行为（`userTokenReadOnly: true`）。
- 可选：如果你希望发出的消息使用当前智能体身份（自定义 `username` 和图标），可添加 `chat:write.customize`。`icon_emoji` 使用 `:emoji_name:` 语法。
...
- `configured_unavailable` 表示该账户通过 SecretRef
  或其他非内联密钥来源完成配置，但当前命令/运行时路径
  无法解析实际值。
- 在 HTTP 模式下，会包含 `signingSecretStatus`；在 Socket Mode 下，
  必需的组合是 `botTokenStatus` + `appTokenStatus`。

<Tip>
对于操作/目录读取，如果配置了用户令牌，则可优先使用用户令牌。对于写入，仍优先使用 bot 令牌；只有在 `userTokenReadOnly: false` 且 bot 令牌不可用时，才允许通过用户令牌写入。
</Tip>
...
<Tabs>
  ...
  <Tab title="渠道策略">
    ...
    运行时说明：如果完全缺少 `channels.slack`（仅通过环境变量设置），运行时会回退为 `groupPolicy="allowlist"` 并记录一条警告（即使已设置 `channels.defaults.groupPolicy`）。

    名称/ID 解析：

    - 在令牌访问允许时，渠道允许列表条目和私信允许列表条目会在启动时解析
    - 无法解析的渠道名称条目会保留配置内容，但默认会在路由时被忽略
    - 入站授权和渠道路由默认优先按 ID；直接用户名/slug 匹配要求 `channels.slack.dangerouslyAllowNameMatching: true`
...
- 适用于直接聊天的旧版回退：`channels.slack.dm.replyToMode`
...
注意：`replyToMode="off"` 会禁用 Slack 中的**所有**回复线程功能，包括显式 `[[reply_to_*]]` 标签。这与 Telegram 不同，后者在 `"off"` 模式下仍会遵循显式标签。该差异反映了平台线程模型的不同：Slack 线程会将消息隐藏在渠道之外，而 Telegram 回复仍会显示在主聊天流中。
...
- 必须存在回复线程，原生文本流式传输才会显示。线程选择仍遵循 `replyToMode`。如果没有线程，则使用普通草稿预览。
- 媒体和非文本负载会回退到常规投递。
- 如果流式传输在回复过程中失败，OpenClaw 会对剩余负载回退到常规投递。
...
`typingReaction` 会在 OpenClaw 处理回复期间，临时为入站 Slack 消息添加一个反应，并在运行结束后移除。在非线程回复场景下，这尤其有用，因为线程回复会使用默认的“正在输入...”状态指示器。
...
<AccordionGroup>
  <Accordion title="入站附件">
    Slack 文件附件会通过 Slack 托管的私有 URL 下载（基于令牌认证的请求流程），并在抓取成功且未超出大小限制时写入媒体存储。
...
- 原生参数菜单现在会自适应其渲染策略：
  - 最多 5 个选项：按钮块
  - 6-100 个选项：静态选择菜单
  - 超过 100 个选项：如果可用交互选项处理器，则使用带异步选项过滤的外部选择
  - 如果编码后的选项值超出 Slack 限制，流程会回退为按钮
- 对于较长的选项负载，斜杠命令参数菜单会在分发所选值前显示确认对话框。
...
Slack 可以充当原生审批客户端，使用交互式按钮和交互流程，而不是回退到 Web UI 或终端。

- 执行审批使用 `channels.slack.execApprovals.*` 进行原生私信/渠道路由。
- 当请求已经落在 Slack 中且审批 ID 类型为 `plugin:` 时，插件审批仍可通过同一个 Slack 原生按钮界面完成。
...
当这些按钮存在时，它们是主要的审批 UX；只有当工具结果表明聊天审批不可用，或手动审批是唯一途径时，OpenClaw
才应包含手动 `/approve` 命令。
...
审批人时，Slack 会自动启用原生执行审批。设置 `enabled: false` 可显式禁用 Slack 作为原生审批客户端。
设置 `enabled: true` 可在审批人已解析时强制启用原生审批。

在没有显式 Slack 执行审批配置时的默认行为：
...
共享 `approvals.exec` 转发是独立功能。仅当执行审批提示还必须
路由到其他聊天或显式的带外目标时才使用它。共享 `approvals.plugin` 转发也
是独立功能；当这些请求已经落在 Slack 中时，Slack 原生按钮仍可处理插件审批。

同聊天内的 `/approve` 在已支持命令的 Slack 渠道和私信中也可使用。完整的审批转发模型见 [执行审批](/zh-CN/tools/exec-approvals)。

## 事件与运行行为

- 消息编辑/删除/线程广播会映射为系统事件。
- 表情添加/移除事件会映射为系统事件。
- 成员加入/离开、渠道创建/重命名，以及固定项添加/移除事件会映射为系统事件。
- 启用 `configWrites` 时，`channel_id_changed` 可迁移渠道配置键。
- 渠道 topic/purpose 元数据被视为不可信上下文，并可注入到路由上下文中。
- 在线程发起者和初始线程历史上下文植入适用时，会根据已配置的发送者允许列表进行过滤。
- 块操作和模态交互会发出结构化的 `Slack interaction: ...` 系统事件，并带有丰富的负载字段：
  - 块操作：选定值、标签、选择器值以及 `workflow_*` 元数据
...
- [配置参考 - Slack](/zh-CN/gateway/configuration-reference#slack)
  ...
  高信号 Slack 字段：
  - 模式/鉴权：`mode`、`botToken`、`appToken`、`signingSecret`、`webhookPath`、`accounts.*`
...
```
  ...
  <Accordion title="Socket 模式无法连接">
    验证 bot + app 令牌，以及 Slack 应用设置中是否已启用 Socket Mode。
