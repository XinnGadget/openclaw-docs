...
x-i18n:
    generated_at: "2026-04-09T01:29:18Z"
    model: gpt-5.4
    provider: openai
...
Questa pagina tratta i **provider LLM/modelli** (non i canali di chat come WhatsApp/Telegram).
...
- Le regole di fallback in fase di esecuzione, i probe di cooldown e la persistenza delle sostituzioni a livello di sessione sono
  documentati in [/concepts/model-failover](/it/concepts/model-failover).
- `models.providers.*.models[].contextWindow` sono metadati nativi del modello;
  `models.providers.*.models[].contextTokens` è il limite effettivo in fase di esecuzione.
...
- I manifest dei provider possono dichiarare `providerAuthEnvVars` e
  ...
  non devono caricare il runtime del plugin. La mappa residua delle env var del core ora è
...
- I plugin provider possono anche gestire il comportamento runtime del provider tramite
  ...
  `onModelSelected`.
- Nota: `capabilities` del runtime provider sono metadati condivisi del runner (famiglia del provider,
  particolarità di transcript/tooling, suggerimenti su transport/cache). Non è la
  stessa cosa del [modello di capability pubblico](/it/plugins/architecture#public-capability-model)
  che descrive ciò che un plugin registra (inferenza di testo, voce, ecc.).

## Comportamento provider gestito dal plugin
...
- `wizard.setup` / `wizard.modelPicker`: il provider gestisce le etichette delle scelte di autenticazione,
  gli alias legacy, i suggerimenti di allowlist per l'onboarding e le voci di configurazione nei selettori di onboarding/modello
...
- `normalizeTransport`: il provider normalizza `api` / `baseUrl` della famiglia di transport
  ...
  poi gli altri plugin provider compatibili con hook finché uno non modifica davvero il
  transport
- `normalizeConfig`: il provider normalizza la configurazione `models.providers.<id>` prima che
  il runtime la usi; OpenClaw controlla prima il provider corrispondente, poi gli altri
  plugin provider compatibili con hook finché uno non modifica davvero la configurazione. Se nessun
  hook provider riscrive la configurazione, gli helper bundled della famiglia Google continuano
  a normalizzare le voci provider Google supportate.
- `applyNativeStreamingUsageCompat`: il provider applica riscritture di compatibilità native per lo streaming usage guidate dall'endpoint per i provider di configurazione
...
- `resolveSyntheticAuth`: il provider può esporre la disponibilità di autenticazione
  locale/self-hosted o altra autenticazione basata sulla configurazione senza persistere segreti in chiaro
- `shouldDeferSyntheticProfileAuth`: il provider può contrassegnare i placeholder dei profili sintetici memorizzati
...
- `prepareDynamicModel`: il provider necessita di un aggiornamento dei metadati prima di ritentare
...
- `resolveReasoningOutputMode`: il provider sceglie i contratti di output del reasoning
  nativi o tagged
...
- `resolveTransportTurnState`: il provider fornisce header o metadati
  nativi del transport per turno
...
- `buildAuthDoctorHint`: il provider aggiunge indicazioni di riparazione quando l'aggiornamento OAuth
  fallisce
- `matchesContextOverflowError`: il provider riconosce errori specifici del provider
  di overflow della finestra di contesto che le euristiche generiche non rileverebbero
- `classifyFailoverReason`: il provider mappa gli errori raw di transport/API specifici del provider
...
- `suppressBuiltInModel`: il provider nasconde le righe upstream obsolete e può restituire un
...
- `applyConfigDefaults`: il provider applica valori predefiniti globali specifici del provider
  durante la materializzazione della configurazione in base a modalità di auth, env o famiglia di modelli
...
- `fetchUsageSnapshot`: il provider gestisce fetch/parsing dell'endpoint di usage mentre
  il core continua a gestire il guscio di riepilogo e la formattazione
...
- `anthropic`: fallback forward-compat Claude 4.6, suggerimenti di riparazione auth, fetch dell'endpoint
  usage, metadati cache-TTL/famiglia provider e valori predefiniti globali della
  configurazione sensibili all'autenticazione
- `amazon-bedrock`: rilevamento dell'overflow del contesto gestito dal provider e classificazione
  dei motivi di failover per errori specifici Bedrock di throttle/non pronto, più
  la famiglia condivisa di replay `anthropic-by-model` per le guardie della policy di replay solo Claude
  sul traffico Anthropic
- `anthropic-vertex`: guardie della policy di replay solo Claude sul traffico
  di messaggi Anthropic
- `openrouter`: id modello pass-through, wrapper di richiesta, suggerimenti sulle capability del provider,
  sanificazione della thought-signature Gemini sul traffico proxy Gemini, iniezione del
  reasoning proxy tramite la famiglia di stream `openrouter-thinking`, inoltro di metadati
  di routing e policy cache-TTL
- `github-copilot`: onboarding/device login, fallback forward-compat del modello,
  suggerimenti di transcript Claude-thinking, scambio di token runtime e fetch dell'endpoint
  usage
- `openai`: fallback forward-compat GPT-5.4, normalizzazione diretta del transport OpenAI,
  suggerimenti di autenticazione mancante sensibili a Codex, soppressione Spark, righe sintetiche
  di catalogo OpenAI/Codex, policy thinking/live-model, normalizzazione degli alias dei token di usage
...
- `google` e `google-gemini-cli`: fallback forward-compat Gemini 3.1,
  ...
  di output reasoning tagged, corrispondenza dei modelli moderni, registrazione bundled del provider
  di generazione immagini per i modelli Gemini image-preview e registrazione bundled del
  provider di generazione video per i modelli Veo; Gemini CLI OAuth inoltre
  gestisce la formattazione del token del profilo auth, il parsing del token di usage e il fetch dell'endpoint quota
  per le superfici di usage
...
- `kilocode`: transport condiviso, header di richiesta gestiti dal plugin, normalizzazione del payload di reasoning,
  sanificazione della thought-signature proxy-Gemini e policy
  cache-TTL
- `zai`: fallback forward-compat GLM-5, valori predefiniti `tool_stream`, policy cache-TTL,
  policy binary-thinking/live-model e auth usage + fetch quota;
...
- `xai`: normalizzazione nativa del transport Responses, riscritture dell'alias `/fast` per
  le varianti Grok fast, valore predefinito `tool_stream`, pulizia specifica xAI di schema degli strumenti /
  payload di reasoning e registrazione bundled del provider di generazione video
...
- `opencode` e `opencode-go`: metadati delle capability gestiti dal plugin più
  sanificazione della thought-signature proxy-Gemini
...
- `fal`: registrazione bundled del provider di generazione video per provider ospitati di terze parti,
...
- `qwen`: cataloghi gestiti dal plugin per modelli di testo più registrazioni condivise del provider
  di media understanding e generazione video per le sue superfici multimodali; la generazione video Qwen usa gli endpoint video DashScope Standard
  con modelli Wan bundled come `wan2.6-t2v` e `wan2.7-r2v`
...
- `minimax`: cataloghi gestiti dal plugin, registrazione bundled del provider di generazione video
  ...
  per `image-01`, selezione ibrida della policy di replay Anthropic/OpenAI
  e logica di auth/snapshot usage
- `together`: cataloghi gestiti dal plugin più registrazione bundled del provider di generazione video
  per i modelli video Wan
- `xiaomi`: cataloghi gestiti dal plugin più logica di auth/snapshot usage
...
- Le route OpenAI native mantengono anche `store` di Responses, suggerimenti di prompt cache e
  model shaping del payload di compatibilità con il reasoning di OpenAI; le route proxy no
...
- Il transport predefinito è `auto` (WebSocket-first, fallback SSE)
- Sostituisci per modello tramite `agents.defaults.models["openai-codex/<model>"].params.transport` (`"sse"`, `"websocket"` o `"auto"`)
...
- `openai-codex/gpt-5.4` mantiene `contextWindow = 1050000` nativo e un `contextTokens = 272000` runtime predefinito; sostituisci il limite runtime con `models.providers.openai-codex.models[].contextTokens`
...
- [MiniMax](/it/providers/minimax): accesso MiniMax Coding Plan via OAuth o chiave API
...
- Attenzione: Gemini CLI OAuth in OpenClaw è un'integrazione non ufficiale. Alcuni utenti hanno segnalato restrizioni dell'account Google dopo aver usato client di terze parti. Controlla i termini di Google e usa un account non critico se scegli di procedere.
- Gemini CLI OAuth viene distribuito come parte del plugin bundled `google`.
  ...
  - Le risposte JSON di Gemini CLI vengono analizzate da `response`; l'usage usa `stats` come fallback, con `stats.cached` normalizzato in `cacheRead` di OpenClaw.
...
- Il catalogo statico di fallback include `kilocode/kilo/auto`; la discovery live di
  `https://api.kilo.ai/api/gateway/models` può espandere ulteriormente il
  catalogo runtime.
...
- OpenClaw applica gli header di attribuzione dell'app documentati da OpenRouter solo quando
  la richiesta è effettivamente diretta a `openrouter.ai`
- I marker `cache_control` Anthropic specifici di OpenRouter sono analogamente limitati
  a route OpenRouter verificate, non a URL proxy arbitrari
- OpenRouter resta sul percorso compatibile con OpenAI in stile proxy, quindi il model shaping
  nativo solo OpenAI (`serviceTier`, `store` di Responses,
  suggerimenti di prompt cache, payload di compatibilità con il reasoning di OpenAI) non viene inoltrato
- I riferimenti OpenRouter basati su Gemini mantengono solo la sanificazione della thought-signature proxy-Gemini;
...
- I riferimenti Kilo basati su Gemini mantengono lo stesso percorso di
  sanificazione della thought-signature proxy-Gemini; `kilocode/kilo/auto` e altri
  suggerimenti incompatibili con il proxy reasoning saltano l'iniezione del proxy reasoning
...
- L'onboarding/configurazione tramite chiave API di MiniMax scrive definizioni esplicite del modello M2.7 con
  `input: ["text", "image"]`; il catalogo provider bundled mantiene i riferimenti chat
  come solo testo finché la configurazione del provider non viene materializzata
...
Molti dei plugin provider bundled sotto pubblicano già un catalogo predefinito.
Usa voci esplicite `models.providers.<id>` solo quando vuoi sostituire il
base URL, gli header o l'elenco dei modelli predefiniti.

### Moonshot AI (Kimi)

Moonshot è distribuito come plugin provider bundled. Usa il provider integrato per
...
Nei selettori di onboarding/configurazione del modello, la scelta auth Volcengine preferisce entrambe le
righe `volcengine/*` e `volcengine-plan/*`. Se questi modelli non sono ancora caricati,
...
Nei selettori di onboarding/configurazione del modello, la scelta auth BytePlus preferisce entrambe le
righe `byteplus/*` e `byteplus-plan/*`. Se questi modelli non sono ancora caricati,
OpenClaw ripiega sul catalogo non filtrato invece di mostrare un selettore
vuoto con ambito provider.
...
Nel percorso di streaming compatibile con Anthropic di MiniMax, OpenClaw disattiva thinking per
...
Ollama è distribuito come plugin provider bundled e usa l'API nativa di Ollama:
...
```bash
# Installa Ollama, quindi scarica un modello:
...
vLLM è distribuito come plugin provider bundled per server
...
Per abilitare l'auto-discovery localmente (qualsiasi valore va bene se il tuo server non impone auth):
...
SGLang è distribuito come plugin provider bundled per server
...
Per abilitare l'auto-discovery localmente (qualsiasi valore va bene se il tuo server non
impone auth):
...
- Le route compatibili con OpenAI in stile proxy saltano anche il model shaping nativo solo OpenAI:
  niente `service_tier`, niente `store` di Responses, niente suggerimenti di prompt cache, niente
  model shaping del payload di compatibilità con il reasoning di OpenAI e nessun header nascosto di attribuzione
  OpenClaw.
