...
summary: Import map, 등록 API 참조, SDK 아키텍처
title: Plugin SDK 개요
x-i18n:
    generated_at: "2026-04-17T06:00:56Z"
    model: gpt-5.4
    provider: openai
...
Plugin SDK는 플러그인과 코어 사이의 타입이 지정된 계약입니다. 이 페이지는 **무엇을 가져와야 하는지**와 **무엇을 등록할 수 있는지**에 대한 참조입니다.

<Tip>
  **사용 방법 가이드를 찾고 있나요?**
  - 첫 번째 플러그인인가요? [Getting Started](/ko/plugins/building-plugins)부터 시작하세요
  - 채널 Plugin인가요? [Channel Plugins](/ko/plugins/sdk-channel-plugins)를 참고하세요
  - 프로바이더 Plugin인가요? [Provider Plugins](/ko/plugins/sdk-provider-plugins)를 참고하세요
</Tip>
...
프로바이더 이름 기반 편의 seam이나 채널 브랜드 헬퍼 seam을 추가하거나
의존하지 마세요. 번들된 플러그인은 자체 `api.ts` 또는 `runtime-api.ts`
barrel 안에서 일반적인 SDK 하위 경로를 조합해야 하며, 코어는 해당 필요가
정말로 채널 간 공통일 때만 플러그인 로컬 barrel을 사용하거나 좁은 범위의
일반 SDK 계약을 추가해야 합니다.

생성된 export map에는 여전히 `plugin-sdk/feishu`, `plugin-sdk/feishu-setup`,
`plugin-sdk/zalo`, `plugin-sdk/zalo-setup`, `plugin-sdk/matrix*` 같은
소수의 번들된 플러그인 헬퍼 seam이 포함되어 있습니다. 이러한 하위 경로는
오직 번들된 플러그인 유지보수와 호환성을 위해 존재하며, 아래의 공통 표에는
의도적으로 제외되어 있고 새로운 서드파티 플러그인에 권장되는 가져오기
경로가 아닙니다.

## 하위 경로 참조

가장 자주 사용되는 하위 경로를 용도별로 그룹화했습니다. 생성된 전체
200개 이상의 하위 경로 목록은 `scripts/lib/plugin-sdk-entrypoints.json`에
있습니다.

예약된 번들된 플러그인 헬퍼 하위 경로도 여전히 생성된 목록에 나타납니다.
문서 페이지에서 명시적으로 공개로 안내하지 않는 한, 이를 구현 세부 사항/호환성
표면으로 취급하세요.
...
<AccordionGroup>
  <Accordion title="채널 하위 경로">
    ...
    | `plugin-sdk/telegram-command-config` | 번들된 계약 fallback이 포함된 Telegram 사용자 지정 명령 정규화/검증 헬퍼 |
    | `plugin-sdk/channel-policy` | `resolveChannelGroupRequireMention` |
    | `plugin-sdk/channel-lifecycle` | `createAccountStatusSink` |
    | `plugin-sdk/inbound-envelope` | 공유 인바운드 라우트 + envelope 빌더 헬퍼 |
    ...
    | `plugin-sdk/thread-bindings-runtime` | 스레드 바인딩 생명주기 및 adapter 헬퍼 |
    | `plugin-sdk/agent-media-payload` | 레거시 agent 미디어 payload 빌더 |
    | `plugin-sdk/conversation-runtime` | 대화/스레드 바인딩, pairing, 구성된 바인딩 헬퍼 |
    ...
    | `plugin-sdk/channel-plugin-common` | 공유 채널 Plugin 프렐류드 exports |
    ...
    | `plugin-sdk/interactive-runtime` | 대화형 reply payload 정규화/축소 헬퍼 |
    | `plugin-sdk/channel-inbound` | 인바운드 debounce, mention 매칭, mention-policy 헬퍼, 그리고 envelope 헬퍼 |
    ...
    | `plugin-sdk/channel-secret-runtime` | `collectSimpleChannelFieldAssignments`, `getChannelSurface`, `pushAssignment` 같은 좁은 범위의 secret-contract 헬퍼와 secret target 타입 |
  </Accordion>
  ...
  <Accordion title="프로바이더 하위 경로">
    ...
    | --- | --- |
    | `plugin-sdk/provider-entry` | `defineSingleProviderPluginEntry` |
    | `plugin-sdk/provider-setup` | 엄선된 로컬/셀프 호스팅 프로바이더 setup 헬퍼 |
    ...
    | `plugin-sdk/provider-auth-login` | 프로바이더 플러그인을 위한 공유 대화형 로그인 헬퍼 |
    | `plugin-sdk/provider-env-vars` | 프로바이더 인증 env-var 조회 헬퍼 |
    ...
    | `plugin-sdk/provider-http` | 일반 프로바이더 HTTP/엔드포인트 capability 헬퍼 |
    | `plugin-sdk/provider-web-fetch-contract` | `enablePluginInConfig`와 `WebFetchProviderPlugin` 같은 좁은 범위의 web-fetch config/선택 계약 헬퍼 |
    | `plugin-sdk/provider-web-fetch` | web-fetch 프로바이더 등록/캐시 헬퍼 |
    | `plugin-sdk/provider-web-search-config-contract` | Plugin-enable 연결이 필요하지 않은 프로바이더를 위한 좁은 범위의 web-search config/자격 증명 헬퍼 |
    | `plugin-sdk/provider-web-search-contract` | `createWebSearchProviderContractFields`, `enablePluginInConfig`, `resolveProviderWebSearchPluginConfig`, 범위가 지정된 자격 증명 setter/getter 같은 좁은 범위의 web-search config/자격 증명 계약 헬퍼 |
  ...
  <Accordion title="인증 및 보안 하위 경로">
    ...
    | `plugin-sdk/approval-gateway-runtime` | 공유 승인 Gateway 해결 헬퍼 |
    ...
    | `plugin-sdk/command-surface` | 명령 본문 정규화 및 command-surface 헬퍼 |
    | `plugin-sdk/allow-from` | `formatAllowFromLowercase` |
    | `plugin-sdk/channel-secret-runtime` | 채널/플러그인 secret 표면을 위한 좁은 범위의 secret-contract 수집 헬퍼 |
    | `plugin-sdk/secret-ref-runtime` | secret-contract/config 파싱을 위한 좁은 범위의 `coerceSecretRef` 및 SecretRef 타이핑 헬퍼 |
    | `plugin-sdk/security-runtime` | 공유 신뢰, DM 게이팅, 외부 콘텐츠, secret 수집 헬퍼 |
    | `plugin-sdk/ssrf-policy` | 호스트 allowlist 및 프라이빗 네트워크 SSRF 정책 헬퍼 |
  ...
  <Accordion title="런타임 및 저장소 하위 경로">
    ...
    | `plugin-sdk/channel-runtime-context` | 일반 채널 runtime-context 등록 및 조회 헬퍼 |
    | `plugin-sdk/runtime-store` | `createPluginRuntimeStore` |
    | `plugin-sdk/plugin-runtime` | 공유 플러그인 명령/hook/http/대화형 헬퍼 |
    ...
    | `plugin-sdk/approval-runtime` | exec/Plugin 승인 헬퍼, approval-capability 빌더, auth/profile 헬퍼, 네이티브 라우팅/런타임 헬퍼 |
    | `plugin-sdk/reply-runtime` | 공유 인바운드/reply 런타임 헬퍼, 청킹, 디스패치, Heartbeat, reply planner |
    | `plugin-sdk/reply-dispatch-runtime` | 좁은 범위의 reply 디스패치/마무리 헬퍼 |
    ...
    | `plugin-sdk/status-helpers` | 공유 채널/계정 상태 요약 헬퍼, runtime-state 기본값, issue 메타데이터 헬퍼 |
    ...
    | `plugin-sdk/logging-core` | 서브시스템 로거 및 리댁션 헬퍼 |
    ...
    | `plugin-sdk/device-bootstrap` | 디바이스 bootstrap 및 pairing 토큰 헬퍼 |
    | `plugin-sdk/extension-shared` | 공유 passive-channel, 상태, ambient proxy 헬퍼 primitive |
    | `plugin-sdk/models-provider-runtime` | `/models` 명령/프로바이더 reply 헬퍼 |
    | `plugin-sdk/skill-commands-runtime` | Skills 명령 목록 헬퍼 |
    | `plugin-sdk/native-command-registry` | 네이티브 명령 레지스트리/빌드/직렬화 헬퍼 |
    | `plugin-sdk/agent-harness` | 저수준 agent harness를 위한 실험적 trusted-plugin 표면: harness 타입, active-run steer/abort 헬퍼, OpenClaw 도구 브리지 헬퍼, 시도 결과 유틸리티 |
    ...
    | `plugin-sdk/fetch-runtime` | 래핑된 fetch, proxy, pinned lookup 헬퍼 |
    | `plugin-sdk/host-runtime` | 호스트명 및 SCP 호스트 정규화 헬퍼 |
    | `plugin-sdk/retry-runtime` | 재시도 config 및 재시도 실행기 헬퍼 |
    | `plugin-sdk/agent-runtime` | agent 디렉터리/ID/workspace 헬퍼 |
  ...
  <Accordion title="Capability 및 테스트 하위 경로">
    | Subpath | 주요 exports |
    | --- | --- |
    | `plugin-sdk/media-runtime` | 공유 미디어 fetch/변환/저장 헬퍼와 미디어 payload 빌더 |
    | `plugin-sdk/media-generation-runtime` | 공유 미디어 생성 failover 헬퍼, 후보 선택, 누락된 모델 메시지 |
    | `plugin-sdk/media-understanding` | 미디어 이해 프로바이더 타입과 프로바이더용 이미지/오디오 헬퍼 export |
    | `plugin-sdk/text-runtime` | assistant-visible-text 제거, markdown 렌더/청킹/표 헬퍼, 리댁션 헬퍼, directive-tag 헬퍼, safe-text 유틸리티 같은 공유 텍스트/markdown/로깅 헬퍼 |
    | `plugin-sdk/text-chunking` | 아웃바운드 텍스트 청킹 헬퍼 |
    | `plugin-sdk/speech` | 음성 프로바이더 타입과 프로바이더용 directive, 레지스트리, 검증 헬퍼 |
    ...
    | `plugin-sdk/image-generation-core` | 공유 이미지 생성 타입, failover, auth, 레지스트리 헬퍼 |
    | `plugin-sdk/music-generation` | 음악 생성 프로바이더/요청/결과 타입 |
    | `plugin-sdk/music-generation-core` | 공유 음악 생성 타입, failover 헬퍼, 프로바이더 조회, model-ref 파싱 |
    | `plugin-sdk/video-generation` | 비디오 생성 프로바이더/요청/결과 타입 |
    | `plugin-sdk/video-generation-core` | 공유 비디오 생성 타입, failover 헬퍼, 프로바이더 조회, model-ref 파싱 |
  ...
  <Accordion title="메모리 하위 경로">
    ...
    | `plugin-sdk/memory-core-host-engine-foundation` | 메모리 호스트 foundation 엔진 exports |
    | `plugin-sdk/memory-core-host-engine-embeddings` | 메모리 호스트 임베딩 계약, 레지스트리 액세스, 로컬 프로바이더, 일반 배치/원격 헬퍼 |
    | `plugin-sdk/memory-core-host-engine-qmd` | 메모리 호스트 QMD 엔진 exports |
    | `plugin-sdk/memory-core-host-engine-storage` | 메모리 호스트 저장소 엔진 exports |
  ...
  <Accordion title="예약된 번들 헬퍼 하위 경로">
    ...
    | 인증/플러그인별 헬퍼 | `plugin-sdk/github-copilot-login`, `plugin-sdk/github-copilot-token`, `plugin-sdk/diagnostics-otel`, `plugin-sdk/diffs`, `plugin-sdk/llm-task`, `plugin-sdk/thread-ownership`, `plugin-sdk/voice-call` | 번들된 기능/플러그인 헬퍼 seam; `plugin-sdk/github-copilot-token`은 현재 `DEFAULT_COPILOT_API_BASE_URL`, `deriveCopilotApiBaseUrlFromToken`, `resolveCopilotApiToken`을 export합니다 |
  </Accordion>
</AccordionGroup>
...
| `api.registerAgentHarness(...)`                  | 실험적 저수준 agent 실행기        |
...
| `api.registerWebFetchProvider(...)`              | 웹 fetch / scrape 프로바이더      |
...
`operator.admin`으로 유지됩니다. 플러그인 소유 메서드에는 플러그인별 접두사를
우선 사용하세요.
...
- `descriptors`: 루트 CLI 도움말, 라우팅, 지연 플러그인 CLI 등록에 사용되는
  파싱 시점 명령 descriptor
...
이 eager 호환성 경로는 계속 지원되지만, 파싱 시점 지연 로드를 위한
...
- 백엔드 `id`는 `codex-cli/gpt-5` 같은 model ref에서 프로바이더 접두사가 됩니다.
...
- 백엔드에 병합 후 호환성 재작성(예: 이전 플래그 형태 정규화)이 필요하면
...
| Method                                     | 등록 대상                                                                                                                                           |
| ------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| `api.registerContextEngine(id, factory)`   | 컨텍스트 엔진(한 번에 하나만 활성). `assemble()` 콜백은 엔진이 프롬프트 추가 구성을 조정할 수 있도록 `availableTools`와 `citationsMode`를 받습니다. |
...
- `registerMemoryCapability`가 권장되는 독점 메모리 플러그인 API입니다.
- `registerMemoryCapability`는 companion 플러그인이 특정 메모리 플러그인의 private
  레이아웃에 접근하지 않고 `openclaw/plugin-sdk/memory-host-core`를 통해
  export된 메모리 아티팩트를 소비할 수 있도록 `publicArtifacts.listArtifacts(...)`도
  노출할 수 있습니다.
...
- `registerMemoryEmbeddingProvider`를 사용하면 활성 메모리 플러그인이 하나 이상의
  임베딩 adapter ID(예: `openai`, `gemini`, 또는 플러그인 정의 사용자 지정 ID)를
  등록할 수 있습니다.
- `agents.defaults.memorySearch.provider` 및
  `agents.defaults.memorySearch.fallback` 같은 사용자 config는 등록된 해당
  adapter ID에 대해 해결됩니다.

### 이벤트 및 생명주기

| Method                                       | 수행 작업                  |
| -------------------------------------------- | -------------------------- |
| `api.on(hookName, handler, opts?)`           | 타입이 지정된 생명주기 hook |
| `api.onConversationBindingResolved(handler)` | 대화 바인딩 콜백            |

### Hook 결정 의미론

- `before_tool_call`: `{ block: true }`를 반환하면 종료됩니다. 하나의 핸들러라도 이를 설정하면 우선순위가 더 낮은 핸들러는 건너뜁니다.
- `before_tool_call`: `{ block: false }`를 반환하면 결정 없음으로 처리됩니다(`block`을 생략한 것과 동일). override가 아닙니다.
- `before_install`: `{ block: true }`를 반환하면 종료됩니다. 하나의 핸들러라도 이를 설정하면 우선순위가 더 낮은 핸들러는 건너뜁니다.
- `before_install`: `{ block: false }`를 반환하면 결정 없음으로 처리됩니다(`block`을 생략한 것과 동일). override가 아닙니다.
- `reply_dispatch`: `{ handled: true, ... }`를 반환하면 종료됩니다. 하나의 핸들러라도 디스패치를 가져가면 우선순위가 더 낮은 핸들러와 기본 모델 디스패치 경로는 건너뜁니다.
- `message_sending`: `{ cancel: true }`를 반환하면 종료됩니다. 하나의 핸들러라도 이를 설정하면 우선순위가 더 낮은 핸들러는 건너뜁니다.
- `message_sending`: `{ cancel: false }`를 반환하면 결정 없음으로 처리됩니다(`cancel`을 생략한 것과 동일). override가 아닙니다.
...
| `api.registrationMode`   | `PluginRegistrationMode`  | 현재 로드 모드; `"setup-runtime"`은 가벼운 전체 엔트리 전 시작/setup 구간입니다            |
...
<Warning>
  ...
  자신의 플러그인을 import하지 마세요. 내부 import는 `./api.ts` 또는
  `./runtime-api.ts`를 통해 연결하세요. SDK 경로는 오직 외부 계약입니다.
</Warning>

파사드로 로드되는 번들된 플러그인 공개 표면(`api.ts`, `runtime-api.ts`,
`index.ts`, `setup-entry.ts`, 및 유사한 공개 엔트리 파일)은 이제
OpenClaw가 이미 실행 중이면 활성 런타임 config 스냅샷을 우선 사용합니다.
아직 런타임 스냅샷이 없으면 디스크에서 해결된 config 파일로 대체됩니다.

프로바이더 플러그인은 헬퍼가 의도적으로 프로바이더 전용이고 아직 일반 SDK
하위 경로에 속하지 않을 때 좁은 범위의 플러그인 로컬 계약 barrel도 노출할 수
있습니다. 현재 번들된 예시: Anthropic 프로바이더는 Anthropic 베타 헤더와
`service_tier` 로직을 일반적인 `plugin-sdk/*` 계약으로 끌어올리는 대신
자체 공개 `api.ts` / `contract-api.ts` seam에 Claude 스트림 헬퍼를 유지합니다.

다른 현재 번들된 예시:

- `@openclaw/openai-provider`: `api.ts`는 프로바이더 빌더,
  기본 모델 헬퍼, 실시간 프로바이더 빌더를 export합니다
- `@openclaw/openrouter-provider`: `api.ts`는 프로바이더 빌더와 함께
  온보딩/config 헬퍼를 export합니다

<Warning>
  extension 프로덕션 코드도 `openclaw/plugin-sdk/<other-plugin>` import를
  피해야 합니다. 헬퍼가 정말로 공유되어야 한다면, 두 플러그인을 결합하는 대신
...
- [SDK Migration](/ko/plugins/sdk-migration) — 사용 중단된 표면에서 마이그레이션
