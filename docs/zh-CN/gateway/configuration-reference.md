...
如果某个提供商配置块完全缺失（`channels.<provider>` 不存在），运行时群组策略会回退为 `allowlist`（默认拒绝）并在启动时给出警告。
</Note>
...
- 对投递目标使用 `user:<id>`（私信）或 `channel:<id>`（服务器频道）；裸数字 ID 会被拒绝。
...
- `channels.discord.guilds.<id>.ignoreOtherMentions`（以及频道级覆盖）会丢弃提及其他用户或角色但未提及该 bot 的消息（不含 @everyone/@here）。
...
**反应通知模式：** `off`（无）、`own`（机器人的消息，默认）、`all`（所有消息）、`allowlist`（`guilds.<id>.users` 中的所有消息）。
...
<Accordion title="iMessage SSH wrapper 示例">
...
- 如果你通过 `openclaw channels add`（或渠道新手引导）添加非默认账号，而当前仍在使用单账号顶级渠道配置，OpenClaw 会先将账号作用域的顶级单账号值提升到渠道账号映射中，以便原始账号继续工作。大多数渠道会将它们移动到 `channels.<channel>.accounts.default`；Matrix 则可以保留现有匹配的命名/default 目标。
...
许多扩展渠道使用 `channels.<id>` 进行配置，并在各自专属渠道页面中记录（例如 Feishu、Matrix、LINE、Nostr、Zalo、Nextcloud Talk、Synology Chat 和 Twitch）。
...
- `bash: true` 启用主机 shell 的 `! <cmd>`。需要 `tools.elevated.enabled`，且发送者位于 `tools.elevated.allowFrom.<channel>` 中。
...
- 对多账号渠道，`channels.<provider>.accounts.<id>.configWrites` 也会控制针对该账号的写入（例如 `/allowlist --config --account <id>` 或 `/config set channels.<provider>.accounts.<id>...`）。
...
- 启动默认值定义在 `scripts/sandbox-browser-entrypoint.sh` 中，并针对容器主机进行了调优：
  ...
  - `--renderer-process-limit=2` 可通过
    `OPENCLAW_BROWSER_RENDERER_PROCESS_LIMIT=<N>` 更改；设为 `0` 则使用 Chromium 的
    默认进程限制。
...
- 文件会在子 workspace 中实体化到 `.openclaw/attachments/<uuid>/`，并附带 `.manifest.json`。
- 附件内容会自动从转录持久化中脱敏。
...
- `install.nodeManager`：用于 `metadata.openclaw.install`
  规格的 node 安装器偏好
  （`npm` | `pnpm` | `yarn` | `bun`）。
- `entries.<skillKey>.enabled: false`：即使是内置/已安装 Skill，也会禁用该 Skill。
...
- `plugins.entries.<id>.hooks.allowPromptInjection`：为 `false` 时，core 会阻止 `before_prompt_build`，并忽略旧版 `before_agent_start` 中会修改 prompt 的字段，同时保留旧版 `modelOverride` 和 `providerOverride`。适用于原生插件 hooks 和受支持 bundle 提供的 hook 目录。
...
- `channels.<provider>.healthMonitor.enabled`：在保持全局监控启用的同时，对单个渠道选择退出健康监控重启。
- `channels.<provider>.accounts.<accountId>.healthMonitor.enabled`：多账号渠道的按账号覆盖。若设置，会优先于渠道级覆盖。
