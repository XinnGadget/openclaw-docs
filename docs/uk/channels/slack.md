---
read_when:
    - Налаштування Slack або налагодження режиму сокета/HTTP Slack
summary: Налаштування Slack і поведінка під час виконання (Socket Mode + URL-адреси HTTP-запитів)
title: Slack
x-i18n:
    generated_at: "2026-04-12T06:23:28Z"
    model: gpt-5.4
    provider: openai
    source_hash: bb9443859b5b1479e25dcf0972940a611b233de55b11c5a6bbfe1f7c5c135cb4
...
Статус: готовий до продакшну для приватних повідомлень + каналів через інтеграції застосунку Slack. Режим за замовчуванням — Socket Mode; URL-адреси HTTP-запитів також підтримуються.

<CardGroup cols={3}>
  <Card title="Підключення" icon="link" href="/uk/channels/pairing">
    Для приватних повідомлень Slack за замовчуванням використовується режим підключення.
  </Card>
...
```json
    "slash_commands": [
      ...
      {
        "command": "/agentstatus",
        "description": "Показати статус під час виконання, включно з використанням/квотою провайдера, якщо доступно",
        "url": "https://gateway-host.example.com/slack/events"
      },
      {
        "command": "/tasks",
...
```
  ...
  <Accordion title="Необов’язкові області доступу для авторства (операції запису)">
    Додайте область доступу бота `chat:write.customize`, якщо хочете, щоб у вихідних повідомленнях використовувалася активна ідентичність агента (власне ім’я користувача та значок) замість стандартної ідентичності застосунку Slack.
...
- `configured_unavailable` означає, що обліковий запис налаштовано через SecretRef
  або інше не-inline джерело секретів, але поточна команда/шлях під час виконання
  не змогли визначити фактичне значення.
- У режимі HTTP включається `signingSecretStatus`; у Socket Mode обов’язковою
  парою є `botTokenStatus` + `appTokenStatus`.

<Tip>
Для дій/читання каталогу можна надавати перевагу токену користувача, якщо його налаштовано. Для запису пріоритетним залишається токен бота; запис через токен користувача дозволено лише коли `userTokenReadOnly: false` і токен бота недоступний.
</Tip>
...
<Tabs>
  <Tab title="Політика приватних повідомлень">
    ...
    - `pairing` (типово)
    - `allowlist`
    - `open` (потребує, щоб `channels.slack.allowFrom` включав `"*"`; застаріле: `channels.slack.dm.allowFrom`)
    - `disabled`
  ...
  <Tab title="Політика каналів">
    ...
    Allowlist каналів розташовано в `channels.slack.channels`, і він має використовувати стабільні ідентифікатори каналів.
    ...
    Визначення назв/ідентифікаторів:

    - записи allowlist каналів і записи allowlist приватних повідомлень визначаються під час запуску, якщо доступ до токена це дозволяє
    - нерозпізнані записи назв каналів зберігаються як налаштовані, але типово ігноруються для маршрутизації
    - вхідна авторизація та маршрутизація каналів типово спираються насамперед на ID; пряме зіставлення за іменем користувача/slug потребує `channels.slack.dangerouslyAllowNameMatching: true`
  ...
  <Tab title="Згадки та користувачі каналу">
    ...
    - неявна поведінка відповіді в потоці на бота (вимикається, коли `thread.requireExplicitMention` дорівнює `true`)

    Налаштування для кожного каналу (`channels.slack.channels.<id>`; назви — лише через визначення під час запуску або `dangerouslyAllowNameMatching`):
...
- `channels.slack.thread.initialHistoryLimit` керує тим, скільки наявних повідомлень потоку отримується під час запуску нової сесії потоку (типово `20`; установіть `0`, щоб вимкнути).
- `channels.slack.thread.requireExplicitMention` (типово `false`): коли `true`, пригнічує неявні згадки в потоці, тому бот відповідає лише на явні згадки `@bot` усередині потоків, навіть якщо бот уже брав участь у потоці. Без цього відповіді в потоці за участю бота обходять обмеження `requireMention`.

Керування прив’язкою відповідей до потоку:
...
Примітка: `replyToMode="off"` вимикає **усі** відповіді з прив’язкою до потоку в Slack, включно з явними тегами `[[reply_to_*]]`. Це відрізняється від Telegram, де явні теги все ще враховуються в режимі `"off"`. Різниця відображає моделі потоків платформи: у Slack потоки приховують повідомлення з каналу, тоді як відповіді в Telegram залишаються видимими в основному потоці чату.
...
- Для нативної потокової передачі тексту та появи статусу потоку помічника Slack має бути доступний потік відповіді. Вибір потоку й надалі підпорядковується `replyToMode`.
- Кореневі повідомлення в каналах і групових чатах усе ще можуть використовувати звичайний чернетковий попередній перегляд, якщо нативна потокова передача недоступна.
- Верхньорівневі приватні повідомлення Slack типово залишаються поза потоком, тому не показують попередній перегляд у стилі потоку; використовуйте відповіді в потоці або `typingReaction`, якщо хочете бачити прогрес там.
- Медіа та не текстові корисні навантаження повертаються до звичайної доставки.
...
## Резервний варіант реакції набору

`typingReaction` додає тимчасову реакцію до вхідного повідомлення Slack, поки OpenClaw обробляє відповідь, а потім видаляє її після завершення запуску. Це особливо корисно поза відповідями в потоці, де використовується типовий індикатор статусу «друкує...».
...
<AccordionGroup>
  <Accordion title="Вхідні вкладення">
    ...
    Типове обмеження розміру вхідних даних під час виконання становить `20MB`, якщо не перевизначено через `channels.slack.mediaMaxMb`.

  </Accordion>

  <Accordion title="Вихідний текст і файли">
    - текстові частини використовують `channels.slack.textChunkLimit` (типово 4000)
    - `channels.slack.chunkMode="newline"` вмикає розбиття насамперед за абзацами
    - надсилання файлів використовує API завантаження Slack і може включати відповіді в потоці (`thread_ts`)
    - обмеження на вихідні медіа підпорядковується `channels.slack.mediaMaxMb`, якщо його налаштовано; інакше надсилання в канали використовує типові значення MIME-kind з конвеєра медіа
  </Accordion>
...
Вбудовані команди потребують [змін у маніфесті](#optional-native-slash-commands) вашого застосунку Slack і вмикаються через `channels.slack.commands.native: true` або замість цього через `commands.native: true` у глобальних конфігураціях.
...
Вбудовані меню аргументів використовують адаптивну стратегію рендерингу, яка показує модальне вікно підтвердження перед відправленням вибраного значення параметра:
...
- перевищено ліміти Slack: закодовані значення параметрів переходять до резервного варіанта з кнопками

```txt
/think
```

Сесії слеш-команд використовують ізольовані ключі на зразок `agent:<agentId>:slack:slash:<userId>` і все одно маршрутизують виконання команд до цільової сесії розмови за допомогою `CommandTargetSessionKey`.
...
- Якщо згенеровані інтерактивні блоки перевищать обмеження Slack Block Kit, OpenClaw повернеться до оригінальної текстової відповіді замість надсилання недійсного корисного навантаження blocks.

## Погодження exec у Slack

Slack може діяти як нативний клієнт погодження з інтерактивними кнопками та взаємодіями замість переходу до резервного варіанта з веб-інтерфейсом або терміналом.

- Погодження exec використовують `channels.slack.execApprovals.*` для нативної маршрутизації приватних повідомлень/каналів.
- Погодження plugin також можуть визначатися через ту саму нативну поверхню кнопок Slack, коли запит уже потрапляє в Slack і тип id погодження — `plugin:`.
- Авторизація погоджувача все одно застосовується: лише користувачі, ідентифіковані як погоджувачі, можуть погоджувати або відхиляти запити через Slack.
...
Типова поведінка без явної конфігурації погодження exec у Slack:
...
налаштовується окремо; нативні кнопки Slack все одно можуть визначати погодження plugin, коли ці запити вже потрапляють
у Slack.

`/approve` у тому самому чаті також працює в каналах і приватних повідомленнях Slack, які вже підтримують команди. Див. [Погодження exec](/uk/tools/exec-approvals), щоб ознайомитися з повною моделлю переспрямування погоджень.

## Події та операційна поведінка

- Редагування/видалення повідомлень/розсилки в потоках зіставляються із системними подіями.
...
- Метадані теми/призначення каналу розглядаються як недовірений контекст і можуть впроваджуватися в контекст маршрутизації.
- Початкове повідомлення потоку та початкове заповнення контексту історії потоку фільтруються за налаштованими allowlist відправників, коли це застосовно.
- Дії блоків і модальні взаємодії створюють структуровані системні події `Slack interaction: ...` з насиченими полями корисного навантаження:
  ...
  - події модальних вікон `view_submission` і `view_closed` з маршрутизованими метаданими каналу та полями введення форми
...
```
  ...
  <Accordion title="Повідомлення в приватних повідомленнях ігноруються">
    ...
    - погодження підключення / записи allowlist
...
```
  ...
  <Accordion title="Режим сокета не підключається">
    Перевірте токени бота й застосунку та те, що Socket Mode увімкнено в налаштуваннях застосунку Slack.
  ...
  <Accordion title="Режим HTTP не отримує події">
    ...
    облікового запису, це означає, що HTTP-обліковий запис налаштовано, але поточне середовище виконання не змогло
