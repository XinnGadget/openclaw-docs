...
A fase leve ingere sinais recentes de memória diária e rastros de recordação, remove duplicatas
...
de memória diária e rastros de recordação. Conteúdo pessoal e sensível é redigido
antes da ingestão.
...
A interface Control expõe o mesmo fluxo de preenchimento retroativo/reset do diário para que você possa inspecionar
...
quais entradas preparadas de curto prazo vieram de replay histórico, quais itens promovidos foram guiados por fundamentação e limpar apenas entradas preparadas fundamentadas sem
...
Pré-visualize reflexões REM, verdades candidatas e a saída da promoção profunda sem
gravar nada:
...
A política de fases, os limites e o comportamento de armazenamento são detalhes internos
de implementação (não são configuração voltada ao usuário).

Consulte a [referência de configuração de memória](/pt-BR/reference/memory-config#dreaming)
