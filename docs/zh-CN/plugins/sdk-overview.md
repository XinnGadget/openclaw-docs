...
带渠道品牌的辅助接缝。内置插件应在其自己的 `api.ts` 或 `runtime-api.ts`
...
- 后端 `id` 会成为模型引用中像 `codex-cli/gpt-5` 这样的提供商前缀。
...
- `registerMemoryEmbeddingProvider` 允许活动内存插件注册一个
  或多个嵌入适配器 id（例如 `openai`、`gemini` 或插件自定义 id）。
...
- [插件内部机制](/zh-CN/plugins/architecture) — 深度架构和能力模型
